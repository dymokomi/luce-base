...
- `T*` converts implicitly to `const T*`, `volatile T*`, and `const volatile T*`. A qualifier is never removed implicitly. Any object pointer converts implicitly to `void*`, and any `const` object pointer to `const void*`. The reverse directions are casts. A nullable pointer `T*?` converts as `T*` does, implicitly to `void*?` or gaining `const`, and by cast to any other nullable pointer; `none` stays `none`.
...
`luce-base bind header.h [-I DIR] [-D NAME[=VALUE]] [--recipe FILE] -o module.lucb [--shims shims.c]` writes a Base module from a C header and reports, one line each with its place, what it could not map. The header is read as the host C compiler reads it: the compiler's include directories and predefined macros are asked of it once, and the preprocessing and the declaration parsing are the tool's own; Clang is not a dependency. The module holds the declarations of the header's own directory and everything beneath it, so `SDL3/SDL.h` gives all of SDL3, and the types of the system headers it includes come through as Base's own. The **recipe** is a file beside the header that states what the header does not, one line each: `nonnull FUNCTION.PARAM`, `nonnull FUNCTION.return`, or `nonnull TYPE.FIELD` for a pointer that is never null; `skip NAME` for a declaration or macro to leave out; `macro PROTOTYPE;`, a C prototype for a function-like macro, which is then reached through a shim.

- Functions, structs, unions, enums, typedefs, pointers, arrays, function pointers, `const`, `_Noreturn`, `_Nonnull`, `__asm__` symbol names, `extern` variables, `static const` scalars, constant `#define`s, and `static inline` functions map directly. The last, and the recipe's macros, are called through `luce_NAME` functions the tool writes into `shims.c`, which the manifest names as a source (§17.4).
- Types map to §5.2: `int` is `i32`, `long` is `c.long`, `char` is `c.char`, `size_t` is `usize`, the `<stdint.h>` names are the Base widths, `va_list` is `c.va_list`. A pointer is nullable, `T*?`, unless the header says `_Nonnull` or the recipe says `nonnull`; `const char*` is `c.str?` and `char*` is `c.char*?`; a pointer to a struct that another library's header defines is `void*?`. A struct the header only names is an `extern type`, a handle. A function pointer is `func(...) -> R`, nullable as `(func(...) -> R)?`; a typedef of one is an alias, `Name?` where it is used.
...
- Function-like macros without a recipe line, `_Generic`, `long double`, `_Complex`, `__int128`, `_BitInt`, `typeof`, bit-fields, and Objective-C declarations are reported and left out; an Objective-C `@interface` is skipped whole.
- `errno`, `stdin`, `stdout`, and `stderr` are macros over accessors and are not bound; `c.errno()` and `c.stdout()` stand for them.

Generated files are ordinary source, checked in and reviewed; the gate writes them again from the installed headers before building the programs on them, so the checked-in copy is what this host last saw. `luce-base bind HEADER --preprocess` prints the preprocessed tokens and `--declarations` the declarations as C spells them, for looking at what the tool read.
...
identity and the current consumer limits. Both compilers and their fixtures are
updated together when the format changes.
...
A build's object is a function of what went into it: every source the compiler read (the standard modules, the program's modules, its manifest), the target and its instruction-set level, the backend and the flags (and, for a C build, the C compiler `CC` names and `LUCE_CFLAGS`), and the compiler itself. `luce build` and `luce test` keep the object under a key that hashes all of these, in `--cache-dir DIR`, else the directory `LUCE_CACHE` names, else `~/.cache/luce-base` (`%LOCALAPPDATA%\.cache\luce-base` on Windows); `none` builds without a cache. A build whose key is in the cache checks the program and links the kept object, translating and assembling nothing; any other build translates, assembles, and keeps its object. `--cache-report` says which happened. An object is written beside its place and renamed, so a concurrent build never reads a partial one. `--emit`, `--lib`, and the manifest's own C sources are not cached. The cache only grows; removing its directory is always safe.
