...
| 7. Proving | programs big enough to break things: a threaded HTTP server, a terminal editor, `luce-base-d` (a debugger), an SDL3 editor; built natively, with no C in the path | in progress: `programs/http`, `programs/editor`, and `programs/debugger` (`luce-base-d`) are under the gate; the first rounds found pointer arithmetic, negated literals, calls through function-typed fields, spinning locks, padding in aggregate equality, a two-register result clobbering its neighbour, and unknown escapes accepted, all pinned in `luce-seed/testdata/programs/` or `samples/errors/` |
