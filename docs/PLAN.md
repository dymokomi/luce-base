...
| 8. Codegen | register allocation over the IR, then the release | in progress: a linear-scan allocator puts integer and float temporaries in callee-saved registers, 27% less assembly; the audit of `docs/AUDIT.md` found ten bugs here and fourteen in the seed, all fixed and pinned; what remains before the release is the missing functionality it lists |
