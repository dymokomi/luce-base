...
| 7. Proving | programs big enough to break things: a threaded HTTP server, a terminal editor, `luce-base-d` (a debugger), an SDL3 editor; built natively, with no C in the path | in progress: `programs/http` is under the gate; the first round found pointer arithmetic, negated literals, calls through function-typed fields, and spinning locks, all pinned in `luce-seed/testdata/programs/` |
