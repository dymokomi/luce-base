...
2. **`luce bind`.** Bindings to C headers are written by hand. Gate: SDL3 and
   ...
   `tests/programs/abi`.
3. **The other native hosts.** arm64-linux has the target model and C emission, no
...
4. **Optimized debugging.** Development DWARF and packaged artifacts are implemented
...
5. **Further library breadth.** TLS belongs to the paused `luce-tls` package. Graphics remains deferred: a portable window/input/GPU
   interface with a proving program on each supported host.
6. **`luce-ld`.** Everything links through the host's `ld` or `cc`. A linker
...
7. **Fuzzing campaigns before a release.** The generator has run for an hour at a time
...
8. **Sanitizers and a litmus suite.** No sanitizer run and no test of the
...
9. **The shape of the compiler, from the audit of 2026-09-14.** The remaining work
    ...
    run by `test.sh`.
10. **Emit only required standard-library code.** Both backends currently emit every
