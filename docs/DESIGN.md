...
`front/lexer.lucb` beside the importing file, else under the root, which is
the entry file's directory or the nearest ancestor where the import resolves,
so a module's own tests run from wherever it sits and a program in a
subdirectory keeps a generated module of its own beside it.
...
| `bind.pre` | the C preprocessor of `luce-base bind`: the host compiler's include directories and predefined macros, then C's directives and macro expansion over a token list |
| `bind.cparse` | C declarations from those tokens: types as trees, records and enums by tag, constant expressions by C's rules |
| `bind.emit` | the declarations as a Base module and `shims.c`, the recipe's answers applied, what cannot map reported |
...
`luce-base bind` is three passes over one arena. `bind.pre` tokenises a header into a
linked list and preprocesses it the way chibicc does: a directive is handled where its
`#` begins a line, an `#include` splices the file's tokens in front of what follows, and a
macro expansion splices its result the same way, each token carrying the set of macros
it must not expand again, so that a self-referential macro stops. The include
directories and the predefined macros are the host compiler's own, asked of it with
`-Wp,-v` and `-dM` once per run, so a header is read exactly as `cc` would read it on
this machine; the `__has_*` queries answer no, except `__has_include` and nullability, so
the tool sees the plainest form of every header. `bind.cparse` reads declarations from
the token list with C's declarator grammar (a nested declarator is read twice, once to
