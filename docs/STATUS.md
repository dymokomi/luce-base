...
| 16.6 | standard `net` HTTP/1.1 and WebSocket primitives | verified | luce-std's `tests/programs/net_protocols`: framing, fragmented input, limits, short writes, independent handshake/frame vectors and malformed input; [contracts and validation](https://github.com/dymokomi/luce-std/blob/main/docs/NET_PROTOCOLS.md) |
...
| — | the optimiser: inlining, single-assignment form, value numbering, load elimination, a register allocator over the exact lives | verified | `tests/optimization`, the native fixpoint; splitting a life at a call is the next step (`docs/PLAN.md`); the allocator's caller-saved integer pool on arm64 is x12–x15 (0.15.0: x12 and x13 joined it, the generator's copies and atomics moved to x16 and x17; the compiler's own build, `src/main.lucb` to assembly on an M-series Mac, went from 4.9 s to 3.9 s and its frame accesses down 3.4%; splitting lives at calls was measured on the same build and gained nothing; 0.17.1: record copies, checked arithmetic, returns and aggregate arguments read their operands and addresses where they live instead of through a scratch register, so the compiler's own arm64 assembly shrank from 1,386,437 to 1,320,604 lines and its register-to-register moves from 126 K to 60 K, on x86-64 from 1,591,929 to 1,536,424 lines and 269 K to 213 K moves; 0.17.2: x0–x7 joined the arm64 caller-saved pool, a call's integer arguments placed by one parallel move, and the `tests/optimization` arm64 counts fell about a tenth; 0.17.4: r8, r9, rsi and rdi joined the x86-64 SysV pool the same way, the compiler's x86-64 assembly 1,509,862 lines and the `tests/optimization` x86-64 counts down about a quarter) |
| — | debugging: native DWARF and `luce-base-d` | limited | `tests/programs/debugger`, `tests/programs/dwarf`: source breakpoints, mixed C/Base stacks, typed locals, scopes, moved artifacts and static-library consumers; `--release --debug` (0.15.0): the optimised program with exact lines and its named locals in frame slots or callee-saved registers, checked in `tests/programs/dwarf` at the same breakpoints; 0.18.0: its lexical scopes and inlined subroutines from the instructions' own scope and site marks, the program's own functions inlined, and payload enums as a tag enumeration with a union of case records, in both debuggers; 0.18.1: the named locals the optimiser lifts into temporaries have location lists, so nothing is pinned |
