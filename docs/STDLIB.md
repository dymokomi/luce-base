...
| `files` | Owned Reader/Writer handles, open modes, seeking/sync, checked close and path/handle metadata; bounded streaming whole-file helpers, owned directory iteration and descriptor-relative lookup/open, checked existence, single-entry mutation, bounded symlink reads, owned temporary files and atomic replacement and bounded copying with publication status | Directory walk, broader filesystem metadata/symlink cases and error detail |
