...
| `files` | Owned Reader/Writer handles, open modes, seeking/sync and checked close; bounded streaming whole-file helpers, owned directory iteration, checked existence, single-entry mutation and bounded symlink reads | Metadata, directory walk, file copying, temporary files, atomic replacement, symlink/path behavior and broader error detail |
