...
| `files` | Owned Reader/Writer handles, open modes, seeking/sync, checked close and path/handle metadata; bounded streaming whole-file helpers, owned directory iteration, checked existence, single-entry mutation, bounded symlink reads, owned temporary files and atomic replacement with publication status | Directory walk, file copying, broader filesystem metadata/symlink cases and error detail |
