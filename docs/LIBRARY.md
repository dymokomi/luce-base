...
- `func absolute_path(path: str) -> interop.Owned[str]!`

- `func path_kind(path: str, follow_symlinks: bool = true) -> FileKind!`
...
- `func init(path: str) -> !`
- `func count() -> i64`
- `func name(index: i64) -> interop.Owned[str]!`
- `mutating func close()`

- `let entries_type: interop.Type[Entries] = interop.Type[Entries]("Directory entries", Entries.close, closeable = true)`
...
- `func init(program: str, arguments: const str[], directory: str = "", output_limit: usize = 1048576, environment: const str[]? = none) -> !`
...
- `func error_message() -> interop.Owned[str]!`
- `func cancel()`
- `mutating func close()`

- `let command_type: interop.Type[Command] = interop.Type[Command]("Command", Command.close, closeable = true)`
...
- `let presentation_in_use: ErrorCode = ErrorCode.package(90)`
...
Exclusive graphics-host lease. Copies alias ownership: destroy one owner on the main thread. Closing the Window invalidates queries immediately, while retaining its native host until this lease is destroyed. This permits graphics resources to be torn down safely in either window/surface destruction order.
...
- `let failed: ErrorCode = ErrorCode.package(117)`

- `func read_text() -> interop.Owned[str]!`
...
- `let unavailable: ErrorCode = ErrorCode.package(92)`

- `let wrong_thread: ErrorCode = ErrorCode.package(93)`

- `let closed: ErrorCode = ErrorCode.package(94)`

- `let invalid_color: ErrorCode = ErrorCode.package(95)`

- `let failed: ErrorCode = ErrorCode.package(96)`
...
An owned device. All GPU operations currently require the main thread. The zero value is closed; copies alias ownership, so borrow and destroy one owner. Surfaces retain the underlying device and remain usable after this handle is destroyed. No window system is initialized by opening a device alone.

- `static func open(backend: Backend = Backend.automatic) -> Device!`
