...
lb_r_gpu_Backend lb_gpu_14select_backend(uint8_t lb_requested) {
    ...
    {
        lb_pos = "src/std/gpu/backend.lucb:13:9";
        if (!!(((lb_requested == ((uint8_t)0u)) || (lb_requested == ((uint8_t)1u))))) 
        {
            lb_pos = "src/std/gpu/backend.lucb:14:13";
            uint8_t _lb_ret7 = ((uint8_t)1u);
            return ((lb_r_gpu_Backend){ .value = _lb_ret7, .failed = false });
        }
    }
    lb_pos = "src/std/gpu/backend.lucb:15:5";
    lb_r_gpu_Backend _lb_err8 = ((lb_r_gpu_Backend){ .error = { .code = (int32_t)(lb_gpu_unsupported), .message = ((lb_str){"the requested GPU backend is not implemented", 44}) }, .failed = true });
    return _lb_err8;
    lb_trap("unreachable");
}
lb_r_void_0p lb_gpu_19backend_device_open(uint8_t lb_backend) {
    ...
    {
        lb_pos = "src/std/gpu/backend.lucb:21:9";
        if (!!((lb_backend == ((uint8_t)1u)))) 
        {
            lb_pos = "src/std/gpu/backend.lucb:22:13";
            void* _lb_ret9 = ((void*)((({ lb_r_gpu_MetalDevice_0p _lb_r10 = lb_gpu_17metal_device_open(); if (_lb_r10.failed) {
                return ((lb_r_void_0p){ .error = _lb_r10.error, .failed = true });
            } _lb_r10.value; }))));
            return ((lb_r_void_0p){ .value = _lb_ret9, .failed = false });
        }
    }
    lb_pos = "src/std/gpu/backend.lucb:23:5";
    lb_r_void_0p _lb_err11 = ((lb_r_void_0p){ .error = { .code = (int32_t)(lb_gpu_unsupported), .message = ((lb_str){"the GPU backend is not implemented", 34}) }, .failed = true });
    return _lb_err11;
    lb_trap("unreachable");
}
void lb_gpu_22backend_device_destroy(uint8_t lb_backend, void* lb_native) {
    ...
    {
        lb_pos = "src/std/gpu/backend.lucb:30:9";
        if (!!((lb_backend == ((uint8_t)1u)))) 
        {
            lb_pos = "src/std/gpu/backend.lucb:31:13";
            (void)(lb_gpu_20metal_device_destroy(((struct lb_gpu_MetalDevice*)(lb_native))));
            lb_pos = "src/std/gpu/backend.lucb:32:13";
            return;
        }
    }
    lb_pos = "src/std/gpu/backend.lucb:33:5";
    (void)(lb_trap_text(((lb_str){"invalid GPU backend during device destruction", 45})));
}
__attribute__((weak)) void lb_interop_Packet_0g1_str_init(lb_interop_Packet_0g1_str* self, lb_str lb_value, void* lb_storage, lb_fn_0F1_void_0p_unit lb_dispose) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
