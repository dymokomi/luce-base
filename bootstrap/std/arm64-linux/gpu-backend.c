...
lb_r_gpu_Backend lb_gpu_14select_backend(uint8_t lb_requested) {
    ...
    lb_r_gpu_Backend _lb_err7 = ((lb_r_gpu_Backend){ .error = { .code = (int32_t)(lb_gpu_unsupported), .message = ((lb_str){"the requested GPU backend is not implemented", 44}) }, .failed = true });
    return _lb_err7;
    lb_trap("unreachable");
}
lb_r_void_0p lb_gpu_19backend_device_open(uint8_t lb_backend) {
    ...
    lb_r_void_0p _lb_err8 = ((lb_r_void_0p){ .error = { .code = (int32_t)(lb_gpu_unsupported), .message = ((lb_str){"the GPU backend is not implemented", 34}) }, .failed = true });
    return _lb_err8;
    lb_trap("unreachable");
}
void lb_gpu_22backend_device_destroy(uint8_t lb_backend, void* lb_native) {
