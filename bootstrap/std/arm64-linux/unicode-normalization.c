...
lb_r_str lb_unicode_normalize(lb_str lb_text, uint8_t lb_form, lb_o_usize lb_max_bytes) {
    ...
    { lb_span _lb_s25 = lb_scalars; lb_release_call(lb_memory_allocator, (lb_span){ (void*)(_lb_s25.data), lb_mul_u(_lb_s25.length, sizeof(uint32_t), 64) }); }
    return ((lb_r_str){ .value = _lb_ret21, .failed = false });
    { lb_span _lb_s26 = lb_scalars; lb_release_call(lb_memory_allocator, (lb_span){ (void*)(_lb_s26.data), lb_mul_u(_lb_s26.length, sizeof(uint32_t), 64) }); }
    lb_trap("unreachable");
}
__attribute__((weak)) void lb_interop_Packet_0g1_str_init(lb_interop_Packet_0g1_str* self, lb_str lb_value, void* lb_storage, lb_fn_0F1_void_0p_unit lb_dispose) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
...
lb_r_unit lb_unicode_15canonical_order(lb_span lb_scalars) {
    ...
    { lb_span _lb_s60 = lb_scratch; lb_release_call(lb_memory_allocator, (lb_span){ (void*)(_lb_s60.data), lb_mul_u(_lb_s60.length, sizeof(uint32_t), 64) }); }
    return ((lb_r_unit){ .failed = false });
}
size_t lb_unicode_17compose_canonical(lb_span lb_scalars) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
