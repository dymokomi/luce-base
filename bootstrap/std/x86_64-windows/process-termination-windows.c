...
int32_t lb_process_28termination_console_received(uint32_t lb_event);

extern _Atomic(uint64_t) lb_process_18termination_owners;
extern _Atomic(uint64_t) lb_process_22termination_generation;
extern _Atomic(bool) lb_process_19termination_closing;
...
uint32_t lb_process_17console_interrupt = 0ULL;
uint32_t lb_process_13console_break = 1ULL;
uint32_t lb_process_13console_close = 2ULL;
uint32_t lb_process_14console_logoff = 5ULL;
uint32_t lb_process_16console_shutdown = 6ULL;
uint64_t lb_process_26console_acknowledgement_ms = 4000ULL;
...
lb_r_unit lb_process_27termination_windows_install(void) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    lb_pos = "src/std/process/termination/windows.lucb:28:5";
    if (!!(({ int32_t _lb_sq7 __attribute__((unused)) = lb_x_process_SetConsoleCtrlHandler(lb_process_28termination_console_received, 1LL); int32_t _lb_sq8 __attribute__((unused)) = 0LL; (_lb_sq7 == _lb_sq8); }))) 
    {
        lb_pos = "src/std/process/termination/windows.lucb:29:9";
        lb_r_unit _lb_err9 = ((lb_r_unit){ .error = { .code = (int32_t)(lb_process_failed), .message = ((lb_str){"the console termination handler could not be installed", 54}) }, .failed = true });
        return _lb_err9;
    }
...
void lb_process_27termination_windows_restore(void) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    lb_pos = "src/std/process/termination/windows.lucb:32:5";
    (void)(((void)(lb_x_process_SetConsoleCtrlHandler(lb_process_28termination_console_received, 0LL))));
}
__attribute__((weak)) void lb_interop_Packet_0g1_str_init(lb_interop_Packet_0g1_str* self, lb_str lb_value, void* lb_storage, lb_fn_0F1_void_0p_unit lb_dispose) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
...
int32_t lb_process_28termination_console_received(uint32_t lb_event) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    lb_pos = "src/std/process/termination/windows.lucb:15:5";
    bool lb_closing __attribute__((unused)) = (((lb_event == lb_process_13console_close) || (lb_event == lb_process_14console_logoff)) || (lb_event == lb_process_16console_shutdown));
    lb_pos = "src/std/process/termination/windows.lucb:16:5";
    if (!!((((!lb_closing) && (!(lb_event == lb_process_17console_interrupt))) && (!(lb_event == lb_process_13console_break))))) 
    {
        lb_pos = "src/std/process/termination/windows.lucb:17:9";
        int32_t _lb_ret14 = 0LL;
        return _lb_ret14;
    }
    lb_pos = "src/std/process/termination/windows.lucb:18:5";
    if (!!(lb_closing)) 
    {
        lb_pos = "src/std/process/termination/windows.lucb:19:9";
        (void)((atomic_store_explicit(&(lb_process_19termination_closing), (bool)(true), memory_order_release), (void)0));
    }
    lb_pos = "src/std/process/termination/windows.lucb:20:5";
    (void)(((void)(((uint64_t)atomic_fetch_add_explicit(&(lb_process_22termination_generation), (uint64_t)(1ULL), memory_order_release)))));
    lb_pos = "src/std/process/termination/windows.lucb:21:5";
    if (!!(lb_closing)) 
    {
        lb_pos = "src/std/process/termination/windows.lucb:22:9";
        uint64_t lb_started __attribute__((unused)) = lb_x_process_GetTickCount64();
        lb_pos = "src/std/process/termination/windows.lucb:23:9";
        for (;;) {
            lb_pos = "src/std/process/termination/windows.lucb:23:9";
            if (!((({ uint64_t _lb_sq17 __attribute__((unused)) = ((uint64_t)atomic_load_explicit(&(lb_process_18termination_owners), memory_order_acquire)); uint64_t _lb_sq19 __attribute__((unused)) = 0ULL; (((uint64_t)(_lb_sq17)) > ((uint64_t)(_lb_sq19))); }) && ({ uint64_t _lb_sq20 __attribute__((unused)) = ({ uint64_t _lb_sq21 __attribute__((unused)) = lb_x_process_GetTickCount64(); uint64_t _lb_sq22 __attribute__((unused)) = lb_started; (uint64_t)(lb_subw_u((uint64_t)(_lb_sq21), (uint64_t)(_lb_sq22), 64)); }); uint64_t _lb_sq23 __attribute__((unused)) = lb_process_26console_acknowledgement_ms; (((uint64_t)(_lb_sq20)) < ((uint64_t)(_lb_sq23))); })))) break;
            {
                lb_pos = "src/std/process/termination/windows.lucb:24:13";
                (void)(lb_x_process_Sleep(1ULL));
            }
        }
    }
    lb_pos = "src/std/process/termination/windows.lucb:25:5";
    int32_t _lb_ret24 = 1LL;
    return _lb_ret24;
    lb_trap("unreachable");
}
