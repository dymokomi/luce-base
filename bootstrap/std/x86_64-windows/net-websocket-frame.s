    .weak lb_memory_copy_0g1_u8
    .text

    .p2align 4
    .globl lb_net_15websocket_frame_0init
lb_net_15websocket_frame_0init:
    .seh_proc lb_net_15websocket_frame_0init
    pushq %rbp
    .seh_pushreg %rbp
    subq $176, %rsp
    .seh_stackalloc 176
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rdi, 168(%rbp)
    .seh_savereg %rdi, 168
    movq %rsi, 160(%rbp)
    .seh_savereg %rsi, 160
    movdqu %xmm6, 144(%rbp)
    .seh_savexmm %xmm6, 144
    movdqu %xmm7, 128(%rbp)
    .seh_savexmm %xmm7, 128
    movdqu %xmm8, 112(%rbp)
    .seh_savexmm %xmm8, 112
    movdqu %xmm9, 96(%rbp)
    .seh_savexmm %xmm9, 96
    movdqu %xmm10, 80(%rbp)
    .seh_savexmm %xmm10, 80
    movdqu %xmm11, 64(%rbp)
    .seh_savexmm %xmm11, 64
    movdqu %xmm12, 48(%rbp)
    .seh_savexmm %xmm12, 48
    movdqu %xmm13, 32(%rbp)
    .seh_savexmm %xmm13, 32
    movdqu %xmm14, 16(%rbp)
    .seh_savexmm %xmm14, 16
    movdqu %xmm15, 0(%rbp)
    .seh_savexmm %xmm15, 0
    .seh_endprologue
    movq %rcx, 192(%rbp)
    movq %rdx, 200(%rbp)
    movq %r8, 208(%rbp)
    movq %r9, 216(%rbp)
    movq 168(%rbp), %rdi
    movq 160(%rbp), %rsi
    movdqu 144(%rbp), %xmm6
    movdqu 128(%rbp), %xmm7
    movdqu 112(%rbp), %xmm8
    movdqu 96(%rbp), %xmm9
    movdqu 80(%rbp), %xmm10
    movdqu 64(%rbp), %xmm11
    movdqu 48(%rbp), %xmm12
    movdqu 32(%rbp), %xmm13
    movdqu 16(%rbp), %xmm14
    movdqu 0(%rbp), %xmm15
    leaq 176(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_21websocket_parse_frame
lb_net_21websocket_parse_frame:
    .seh_proc lb_net_21websocket_parse_frame
    pushq %rbp
    .seh_pushreg %rbp
    subq $880, %rsp
    .seh_stackalloc 880
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 696(%rbp)
    movq %rdi, 872(%rbp)
    .seh_savereg %rdi, 872
    movq %rsi, 864(%rbp)
    .seh_savereg %rsi, 864
    movdqu %xmm6, 848(%rbp)
    .seh_savexmm %xmm6, 848
    movdqu %xmm7, 832(%rbp)
    .seh_savexmm %xmm7, 832
    movdqu %xmm8, 816(%rbp)
    .seh_savexmm %xmm8, 816
    movdqu %xmm9, 800(%rbp)
    .seh_savexmm %xmm9, 800
    movdqu %xmm10, 784(%rbp)
    .seh_savexmm %xmm10, 784
    movdqu %xmm11, 768(%rbp)
    .seh_savexmm %xmm11, 768
    movdqu %xmm12, 752(%rbp)
    .seh_savexmm %xmm12, 752
    movdqu %xmm13, 736(%rbp)
    .seh_savexmm %xmm13, 736
    movdqu %xmm14, 720(%rbp)
    .seh_savexmm %xmm14, 720
    movdqu %xmm15, 704(%rbp)
    .seh_savexmm %xmm15, 704
    movq %rbx, 688(%rbp)
    .seh_savereg %rbx, 688
    movq %r12, 680(%rbp)
    .seh_savereg %r12, 680
    movq %r13, 672(%rbp)
    .seh_savereg %r13, 672
    movq %r14, 664(%rbp)
    .seh_savereg %r14, 664
    movq %r15, 656(%rbp)
    .seh_savereg %r15, 656
    .seh_endprologue
    movq %rcx, 896(%rbp)
    movq %rdx, 904(%rbp)
    movq %r8, 912(%rbp)
    movq %r9, 920(%rbp)
    movq 904(%rbp), %r10
    leaq 568(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 912(%rbp), %rax
    movl %eax, 552(%rbp)
    movq 920(%rbp), %rax
    movq %rax, 536(%rbp)
    leaq 568(%rbp), %rbx
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq (%r10), %r12
    movq $2, %rcx
    cmpq %rcx, %r12
    jae .L1_2
.L1_1:
    leaq 496(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movq $0, 32(%r11)
    leaq 584(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 32(%r10), %rax
    movq %rax, 32(%r11)
    movq $64, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_2:
.L1_3:
    movq %rbx, %r10
    movq (%r10), %rax
    movq %rax, 104(%rbp)
    movq $0, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_0(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %r10
    movzbl (%r10), %r14d
    movl $112, %ecx
    andl %ecx, %r14d
    movl $0, %ecx
    cmpl %ecx, %r14d
    sete %al
    movzbl %al, %r14d
    movl $0, %ecx
    cmpl %ecx, %r14d
    jne .L1_6
.L1_5:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_1(%rip), %r13
    leaq 480(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $44, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_6:
.L1_7:
    movq 104(%rbp), %r10
    movzbl (%r10), %ebx
    movl $15, %ecx
    andl %ecx, %ebx
    subq $32, %rsp
    movl %ebx, %eax
    movq %rax, 8(%rsp)
    leaq 440(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_net_16websocket_opcode
    addq $32, %rsp
    leaq 440(%rbp), %r14
    movq $32, %rcx
    movq %r14, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L1_10
    jmp .L1_9
.L1_10:
    movq $8, %rcx
    movq %r14, %rbx
    addq %rcx, %rbx
    leaq 584(%rbp), %r12
    movq $40, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movq %rbx, %r10
    movq %r13, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $64, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_9:
    movq %r14, %r10
    movzbl (%r10), %eax
    movl %eax, 96(%rbp)
    movq 104(%rbp), %r10
    movzbl (%r10), %eax
    movl %eax, 88(%rbp)
    movl 88(%rbp), %eax
    movl $128, %ecx
    andl %ecx, %eax
    movl %eax, 80(%rbp)
    movl 80(%rbp), %eax
    movl $0, %ecx
    cmpl %ecx, %eax
    sete %al
    movzbl %al, %eax
    movl %eax, 72(%rbp)
    movl 72(%rbp), %eax
    movl $0, %ecx
    cmpl %ecx, %eax
    sete %al
    movzbl %al, %eax
    movl %eax, 64(%rbp)
    movq $1, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_4(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %rax
    movq $1, %rcx
    addq %rcx, %rax
    movq %rax, 56(%rbp)
    movq 56(%rbp), %r10
    movzbl (%r10), %ebx
    movl $128, %ecx
    andl %ecx, %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %ebx
    movzbl %bl, %eax
    movl %eax, 48(%rbp)
    leaq 552(%rbp), %r15
    movq %r15, %r10
    movzbl (%r10), %r15d
    movl 48(%rbp), %eax
    cmpl %r15d, %eax
    sete %al
    movzbl %al, %r15d
    movl $0, %ecx
    cmpl %ecx, %r15d
    jne .L1_13
.L1_12:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_5(%rip), %r13
    leaq 424(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $45, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_13:
.L1_14:
    movq 56(%rbp), %r10
    movzbl (%r10), %r14d
    movl $127, %ecx
    andl %ecx, %r14d
    movzbl %r14b, %r14d
    movl %r14d, %eax
    movq %rax, 40(%rbp)
    movl $126, %ecx
    cmpl %ecx, %r14d
    jne .L1_17
.L1_16:
    movq $4, %rcx
    cmpq %rcx, %r12
    jae .L1_20
.L1_19:
    leaq 384(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movq $0, 32(%r11)
    leaq 584(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 32(%r10), %rax
    movq %rax, 32(%r11)
    movq $64, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_20:
.L1_21:
    movq $2, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_7(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %rax
    movq $2, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movl %ebx, %ebx
.L1_24:
    movq $8, %rcx
    movq %rbx, %r14
    shlq %cl, %r14
    movq $3, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_7(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %rax
    movq $3, %rcx
    movq %rax, %r15
    addq %rcx, %r15
    movq %r15, %r10
    movzbl (%r10), %r15d
    movl %r15d, %r15d
    orq %r15, %r14
    movq $126, %rcx
    cmpq %rcx, %r14
    jae .L1_26
.L1_25:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_9(%rip), %r13
    leaq 368(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $35, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_26:
.L1_27:
    movq $4, %rax
    movq %rax, %rbx
    movq %r14, %rax
    movq %rax, 0(%rbp)
    jmp .L1_18
.L1_17:
    movl $127, %ecx
    cmpl %ecx, %r14d
    jne .L1_30
.L1_29:
    movq $10, %rcx
    cmpq %rcx, %r12
    jae .L1_33
.L1_32:
    leaq 328(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movq $0, 32(%r11)
    leaq 584(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 32(%r10), %rax
    movq %rax, 32(%r11)
    movq $64, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_33:
.L1_34:
    movq $2, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_10(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %rax
    movq $2, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movl $128, %ecx
    andl %ecx, %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    jne .L1_37
.L1_36:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_11(%rip), %r13
    leaq 312(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $45, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_37:
.L1_38:
    movq $0, %rax
    movq %rax, %rbx
    movq $2, %rax
    movq %rax, %r14
.L1_40:
    movq $10, %rcx
    cmpq %rcx, %r14
    jge .L1_43
.L1_41:
.L1_45:
    movq $8, %rcx
    movq %rbx, %rax
    shlq %cl, %rax
    movq %rax, 32(%rbp)
    cmpq %r12, %r14
    jb 1f
    leaq .Ltext_12(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 104(%rbp), %rax
    movq %rax, %r15
    addq %r14, %r15
    movq %r15, %r10
    movzbl (%r10), %r15d
    movl %r15d, %r15d
    movq 32(%rbp), %rax
    movq %r15, %rcx
    movq %rax, %r15
    orq %rcx, %r15
.L1_42:
    movq $1, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq %r15, %rbx
    movq %r13, %r14
    jmp .L1_40
.L1_43:
    movq $65536, %rcx
    cmpq %rcx, %rbx
    jae .L1_47
.L1_46:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_9(%rip), %r13
    leaq 296(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $35, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_47:
.L1_48:
    movq $10, %rax
    movq %rax, %r13
    movq %rbx, %r14
    jmp .L1_31
.L1_30:
    movq $2, %rax
    movq %rax, %r13
    movq 40(%rbp), %rax
    movq %rax, %r14
.L1_31:
    movq %r13, %rbx
    movq %r14, %rax
    movq %rax, 0(%rbp)
.L1_18:
    leaq 536(%rbp), %r13
    movq %r13, %r10
    movq (%r10), %r13
    movq 0(%rbp), %rax
    cmpq %r13, %rax
    jbe .L1_51
.L1_50:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_15websocket_limit(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_13(%rip), %r13
    leaq 280(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $38, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_51:
.L1_52:
    movl 96(%rbp), %eax
    movzbl %al, %eax
    movl %eax, 24(%rbp)
    movl 24(%rbp), %eax
    movl %eax, %r14d
    movzbl %r14b, %r14d
    movl $8, %ecx
    cmpl %ecx, %r14d
    setae %al
    movzbl %al, %r14d
    testl %r14d, %r14d
    jne .L1_57
    jmp .L1_72
.L1_72:
    movl %r14d, %r15d
    jmp .L1_58
.L1_57:
    movl 64(%rbp), %eax
    movzbl %al, %r14d
    movl $0, %ecx
    cmpl %ecx, %r14d
    sete %al
    movzbl %al, %r14d
    testl %r14d, %r14d
    jne .L1_73
    jmp .L1_59
.L1_73:
    movl %r14d, %r15d
    jmp .L1_60
.L1_59:
    movq 0(%rbp), %rax
    movq $125, %rcx
    cmpq %rcx, %rax
    seta %al
    movzbl %al, %r15d
.L1_60:
    movzbl %r15b, %r14d
    movl %r14d, %r15d
.L1_58:
    movzbl %r15b, %r14d
    testl %r14d, %r14d
    jne .L1_54
    jmp .L1_55
.L1_54:
    leaq 584(%rbp), %rbx
    movq $40, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_14(%rip), %r13
    leaq 264(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $59, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_55:
.L1_56:
    leaq 260(%rbp), %rax
    movq %rax, 16(%rbp)
    movq 16(%rbp), %r11
    movl $0, 0(%r11)
    movl 48(%rbp), %eax
    testl %eax, %eax
    jne .L1_62
    jmp .L1_63
.L1_62:
    movq %r12, %rax
    movq %rbx, %rcx
    subq %rcx, %rax
    jnc 1f
    leaq .Ltext_15(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r15
    movq $4, %rcx
    cmpq %rcx, %r15
    jae .L1_66
.L1_65:
    leaq 216(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movq $0, 32(%r11)
    leaq 584(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 32(%r10), %rax
    movq %rax, 32(%r11)
    movq $64, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
.L1_66:
.L1_67:
    leaq 200(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 16(%rbp), %rax
    movq 8(%rbp), %r10
    movq %rax, (%r10)
    movq 8(%rbp), %rax
    movq $8, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    movq $4, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $1, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    cmpq %r13, %rbx
    jb 1f
    leaq .Ltext_16(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    cmpq %r13, %r12
    jb 1f
    leaq .Ltext_16(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    cmpq %r12, %rbx
    jbe .L1_69
.L1_70:
    leaq .Ltext_16(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L1_69:
    movq 104(%rbp), %rax
    movq %rax, %r13
    addq %rbx, %r13
    movq %r12, %r14
    subq %rbx, %r14
    leaq 184(%rbp), %r15
    movq %r15, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r15, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq %r14, (%r10)
    subq $64, %rsp
    movq 8(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 0(%rsp)
    movq %r15, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 8(%rsp)
    movq $4, %rax
    movq %rax, 16(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_memory_copy_0g1_u8
    addq $64, %rsp
    movq %rbx, %rax
    movq $4, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_18(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r13
    jmp .L1_64
.L1_63:
    movq %rbx, %r13
.L1_64:
    leaq 152(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movl 24(%rbp), %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movl 64(%rbp), %eax
    movzbl %al, %r12d
    movq $1, %rcx
    movq %rbx, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movb %r12b, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq 0(%rbp), %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $16, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq %r13, (%r10)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq 16(%rbp), %r10
    movq %r12, %r11
    movl 0(%r10), %eax
    movl %eax, 0(%r11)
    movq $28, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl 48(%rbp), %eax
    movq %r12, %r10
    movb %al, (%r10)
    leaq 112(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq $32, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    leaq 584(%rbp), %rbx
    movq %r12, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 32(%r10), %rax
    movq %rax, 32(%r11)
    movq $64, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 696(%rbp), %rdi
    movq $72, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 696(%rbp), %rax
    movq 872(%rbp), %rdi
    movq 864(%rbp), %rsi
    movdqu 848(%rbp), %xmm6
    movdqu 832(%rbp), %xmm7
    movdqu 816(%rbp), %xmm8
    movdqu 800(%rbp), %xmm9
    movdqu 784(%rbp), %xmm10
    movdqu 768(%rbp), %xmm11
    movdqu 752(%rbp), %xmm12
    movdqu 736(%rbp), %xmm13
    movdqu 720(%rbp), %xmm14
    movdqu 704(%rbp), %xmm15
    movq 688(%rbp), %rbx
    movq 680(%rbp), %r12
    movq 672(%rbp), %r13
    movq 664(%rbp), %r14
    movq 656(%rbp), %r15
    leaq 880(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_16websocket_opcode
lb_net_16websocket_opcode:
    .seh_proc lb_net_16websocket_opcode
    pushq %rbp
    .seh_pushreg %rbp
    subq $304, %rsp
    .seh_stackalloc 304
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 120(%rbp)
    movq %rdi, 296(%rbp)
    .seh_savereg %rdi, 296
    movq %rsi, 288(%rbp)
    .seh_savereg %rsi, 288
    movdqu %xmm6, 272(%rbp)
    .seh_savexmm %xmm6, 272
    movdqu %xmm7, 256(%rbp)
    .seh_savexmm %xmm7, 256
    movdqu %xmm8, 240(%rbp)
    .seh_savexmm %xmm8, 240
    movdqu %xmm9, 224(%rbp)
    .seh_savexmm %xmm9, 224
    movdqu %xmm10, 208(%rbp)
    .seh_savexmm %xmm10, 208
    movdqu %xmm11, 192(%rbp)
    .seh_savexmm %xmm11, 192
    movdqu %xmm12, 176(%rbp)
    .seh_savexmm %xmm12, 176
    movdqu %xmm13, 160(%rbp)
    .seh_savexmm %xmm13, 160
    movdqu %xmm14, 144(%rbp)
    .seh_savexmm %xmm14, 144
    movdqu %xmm15, 128(%rbp)
    .seh_savexmm %xmm15, 128
    movq %rbx, 112(%rbp)
    .seh_savereg %rbx, 112
    movq %r12, 104(%rbp)
    .seh_savereg %r12, 104
    movq %r13, 96(%rbp)
    .seh_savereg %r13, 96
    movq %r14, 88(%rbp)
    .seh_savereg %r14, 88
    .seh_endprologue
    movq %rcx, 320(%rbp)
    movq %rdx, 328(%rbp)
    movq %r8, 336(%rbp)
    movq %r9, 344(%rbp)
    movq 328(%rbp), %rax
    movl %eax, 32(%rbp)
    leaq 32(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    leaq 31(%rbp), %r12
    movq %r12, %r10
    movb %bl, (%r10)
    leaq 30(%rbp), %rbx
    movq %r12, %r10
    movq %rbx, %r11
    movzbl 0(%r10), %eax
    movb %al, 0(%r11)
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    je .L2_2
.L2_4:
    jmp .L2_3
.L2_2:
    leaq 48(%rbp), %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_5:
    jmp .L2_1
.L2_3:
    movl $1, %ecx
    cmpl %ecx, %ebx
    je .L2_6
.L2_8:
    jmp .L2_7
.L2_6:
    leaq 48(%rbp), %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_9:
    jmp .L2_1
.L2_7:
    movl $2, %ecx
    cmpl %ecx, %ebx
    je .L2_10
.L2_12:
    jmp .L2_11
.L2_10:
    leaq 48(%rbp), %rbx
    movl $2, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_13:
    jmp .L2_1
.L2_11:
    movl $8, %ecx
    cmpl %ecx, %ebx
    je .L2_14
.L2_16:
    jmp .L2_15
.L2_14:
    leaq 48(%rbp), %rbx
    movl $8, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_17:
    jmp .L2_1
.L2_15:
    movl $9, %ecx
    cmpl %ecx, %ebx
    je .L2_18
.L2_20:
    jmp .L2_19
.L2_18:
    leaq 48(%rbp), %rbx
    movl $9, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_21:
    jmp .L2_1
.L2_19:
    movl $10, %ecx
    cmpl %ecx, %ebx
    je .L2_22
.L2_24:
    jmp .L2_23
.L2_22:
    leaq 48(%rbp), %rbx
    movl $10, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_25:
    jmp .L2_1
.L2_23:
    jmp .L2_26
.L2_28:
    jmp .L2_27
.L2_26:
    leaq 48(%rbp), %rbx
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_net_17websocket_invalid(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_21(%rip), %r13
    leaq 8(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $25, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $32, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $40, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L2_29:
    jmp .L2_1
.L2_27:
.L2_1:
    leaq .Ltext_22(%rip), %rdi
    leaq .Ltext_19(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
    .seh_endproc

    .p2align 4
    .globl lb_net_21websocket_check_close
lb_net_21websocket_check_close:
    .seh_proc lb_net_21websocket_check_close
    pushq %rbp
    .seh_pushreg %rbp
    subq $448, %rsp
    .seh_stackalloc 448
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 264(%rbp)
    movq %rdi, 440(%rbp)
    .seh_savereg %rdi, 440
    movq %rsi, 432(%rbp)
    .seh_savereg %rsi, 432
    movdqu %xmm6, 416(%rbp)
    .seh_savexmm %xmm6, 416
    movdqu %xmm7, 400(%rbp)
    .seh_savexmm %xmm7, 400
    movdqu %xmm8, 384(%rbp)
    .seh_savexmm %xmm8, 384
    movdqu %xmm9, 368(%rbp)
    .seh_savexmm %xmm9, 368
    movdqu %xmm10, 352(%rbp)
    .seh_savexmm %xmm10, 352
    movdqu %xmm11, 336(%rbp)
    .seh_savexmm %xmm11, 336
    movdqu %xmm12, 320(%rbp)
    .seh_savexmm %xmm12, 320
    movdqu %xmm13, 304(%rbp)
    .seh_savexmm %xmm13, 304
    movdqu %xmm14, 288(%rbp)
    .seh_savexmm %xmm14, 288
    movdqu %xmm15, 272(%rbp)
    .seh_savexmm %xmm15, 272
    movq %rbx, 256(%rbp)
    .seh_savereg %rbx, 256
    movq %r12, 248(%rbp)
    .seh_savereg %r12, 248
    movq %r13, 240(%rbp)
    .seh_savereg %r13, 240
    movq %r14, 232(%rbp)
    .seh_savereg %r14, 232
    movq %r15, 224(%rbp)
    .seh_savereg %r15, 224
    .seh_endprologue
    movq %rcx, 464(%rbp)
    movq %rdx, 472(%rbp)
    movq %r8, 480(%rbp)
    movq %r9, 488(%rbp)
    movq 472(%rbp), %r10
    leaq 176(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 176(%rbp), %rbx
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq (%r10), %r12
    movq $0, %rcx
    cmpq %rcx, %r12
    jne .L3_2
.L3_1:
    leaq 192(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
.L3_2:
.L3_3:
    movq $2, %rcx
    cmpq %rcx, %r12
    setb %al
    movzbl %al, %r13d
    testl %r13d, %r13d
    jne .L3_43
    jmp .L3_8
.L3_43:
    movl %r13d, %r14d
    jmp .L3_9
.L3_8:
    movq $125, %rcx
    cmpq %rcx, %r12
    seta %al
    movzbl %al, %r14d
.L3_9:
    movzbl %r14b, %r13d
    testl %r13d, %r13d
    jne .L3_5
    jmp .L3_6
.L3_5:
    leaq 192(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_23(%rip), %r12
    leaq 160(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $38, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
.L3_6:
.L3_7:
    movq %rbx, %r10
    movq (%r10), %r13
    movq $0, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_24(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %r13, %r10
    movzbl (%r10), %r14d
    movl %r14d, %r14d
    movzwl %r14w, %r14d
.L3_12:
    movl $8, %ecx
    movl %r14d, %ebx
    shll %cl, %ebx
    movzwl %bx, %ebx
    movq $1, %rax
    cmpq %r12, %rax
    jb 1f
    leaq .Ltext_24(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $1, %rcx
    movq %r13, %r15
    addq %rcx, %r15
    movq %r15, %r10
    movzbl (%r10), %r15d
    movl %r15d, %r15d
    movzwl %r15w, %r15d
    orl %r15d, %ebx
    movzwl %bx, %ebx
    movl $1000, %ecx
    cmpl %ecx, %ebx
    setb %al
    movzbl %al, %r15d
    testl %r15d, %r15d
    jne .L3_44
    jmp .L3_16
.L3_44:
    movl %r15d, %r14d
    jmp .L3_17
.L3_16:
    movl $5000, %ecx
    cmpl %ecx, %ebx
    setae %al
    movzbl %al, %r14d
.L3_17:
    movzbl %r14b, %r15d
    testl %r15d, %r15d
    jne .L3_45
    jmp .L3_18
.L3_45:
    movl %r15d, %r14d
    jmp .L3_19
.L3_18:
    movl $1004, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %r14d
.L3_19:
    movzbl %r14b, %r15d
    testl %r15d, %r15d
    jne .L3_46
    jmp .L3_20
.L3_46:
    movl %r15d, %r14d
    jmp .L3_21
.L3_20:
    movl $1005, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %r14d
.L3_21:
    movzbl %r14b, %r15d
    testl %r15d, %r15d
    jne .L3_47
    jmp .L3_22
.L3_47:
    movl %r15d, %r14d
    jmp .L3_23
.L3_22:
    movl $1006, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %r14d
.L3_23:
    movzbl %r14b, %r15d
    testl %r15d, %r15d
    jne .L3_48
    jmp .L3_24
.L3_48:
    movl %r15d, %r14d
    jmp .L3_25
.L3_24:
    movl $1016, %ecx
    cmpl %ecx, %ebx
    setae %al
    movzbl %al, %r14d
    testl %r14d, %r14d
    jne .L3_26
    jmp .L3_49
.L3_49:
    movl %r14d, %r15d
    jmp .L3_27
.L3_26:
    movl $3000, %ecx
    cmpl %ecx, %ebx
    setb %al
    movzbl %al, %r15d
.L3_27:
    movzbl %r15b, %r14d
.L3_25:
    movzbl %r14b, %r15d
    testl %r15d, %r15d
    jne .L3_50
    jmp .L3_28
.L3_50:
    movl %r15d, %ebx
    jmp .L3_29
.L3_28:
    movl $1015, %ecx
    cmpl %ecx, %ebx
    sete %al
    movzbl %al, %r14d
    movl %r14d, %ebx
.L3_29:
    movzbl %bl, %r14d
    testl %r14d, %r14d
    jne .L3_13
    jmp .L3_14
.L3_13:
    leaq 192(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_25(%rip), %r12
    leaq 144(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $31, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
.L3_14:
.L3_15:
    leaq 136(%rbp), %rax
    movq %rax, 40(%rbp)
    movq 40(%rbp), %r11
    movq $0, 0(%r11)
    movq $1, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq $2, %rax
    cmpq %r14, %rax
    jb 1f
    leaq .Ltext_26(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    cmpq %r14, %r12
    jb 1f
    leaq .Ltext_26(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $2, %rax
    cmpq %r12, %rax
    jbe .L3_35
.L3_36:
    leaq .Ltext_26(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L3_35:
    movq $2, %rcx
    movq %r13, %rax
    addq %rcx, %rax
    movq %rax, 32(%rbp)
    movq $2, %rcx
    movq %r12, %r15
    subq %rcx, %r15
    leaq 120(%rbp), %rax
    movq %rax, 24(%rbp)
    movq 32(%rbp), %rax
    movq 24(%rbp), %r10
    movq %rax, (%r10)
    movq 24(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 16(%rbp)
    movq 16(%rbp), %r10
    movq %r15, (%r10)
    leaq 80(%rbp), %r14
    movq $32, %rcx
    movq %r14, %rax
    addq %rcx, %rax
    movq %rax, 8(%rbp)
    movq $0, %rax
    movq %rax, %rbx
.L3_31:
    cmpq %r15, %rbx
    jae .L3_34
.L3_32:
    movq 32(%rbp), %rax
    movq %rax, %r12
    addq %rbx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movzbl %r12b, %r12d
    subq $32, %rsp
    movq 40(%rbp), %rax
    movq %rax, 8(%rsp)
    movl %r12d, %eax
    movq %rax, 16(%rsp)
    leaq 80(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_utf8_Decoder_push
    addq $32, %rsp
    movq 8(%rbp), %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L3_38
    jmp .L3_37
.L3_38:
    movq $8, %rcx
    movq %r14, %rbx
    addq %rcx, %rbx
    leaq 192(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
.L3_37:
.L3_33:
    movq $1, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r12, %rbx
    jmp .L3_31
.L3_34:
    subq $32, %rsp
    movq 40(%rbp), %rax
    movq %rax, 8(%rsp)
    leaq 48(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_utf8_Decoder_finish
    addq $32, %rsp
    leaq 48(%rbp), %r12
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L3_41
    jmp .L3_40
.L3_41:
    leaq 192(%rbp), %rbx
    movq %r12, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movl $1, %eax
    movq %r13, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
.L3_40:
    leaq 192(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 264(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 264(%rbp), %rax
    movq 440(%rbp), %rdi
    movq 432(%rbp), %rsi
    movdqu 416(%rbp), %xmm6
    movdqu 400(%rbp), %xmm7
    movdqu 384(%rbp), %xmm8
    movdqu 368(%rbp), %xmm9
    movdqu 352(%rbp), %xmm10
    movdqu 336(%rbp), %xmm11
    movdqu 320(%rbp), %xmm12
    movdqu 304(%rbp), %xmm13
    movdqu 288(%rbp), %xmm14
    movdqu 272(%rbp), %xmm15
    movq 256(%rbp), %rbx
    movq 248(%rbp), %r12
    movq 240(%rbp), %r13
    movq 232(%rbp), %r14
    movq 224(%rbp), %r15
    leaq 448(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketSequence_begin
lb_net_WebSocketSequence_begin:
    .seh_proc lb_net_WebSocketSequence_begin
    pushq %rbp
    .seh_pushreg %rbp
    subq $416, %rsp
    .seh_stackalloc 416
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 232(%rbp)
    movq %rdi, 408(%rbp)
    .seh_savereg %rdi, 408
    movq %rsi, 400(%rbp)
    .seh_savereg %rsi, 400
    movdqu %xmm6, 384(%rbp)
    .seh_savexmm %xmm6, 384
    movdqu %xmm7, 368(%rbp)
    .seh_savexmm %xmm7, 368
    movdqu %xmm8, 352(%rbp)
    .seh_savexmm %xmm8, 352
    movdqu %xmm9, 336(%rbp)
    .seh_savexmm %xmm9, 336
    movdqu %xmm10, 320(%rbp)
    .seh_savexmm %xmm10, 320
    movdqu %xmm11, 304(%rbp)
    .seh_savexmm %xmm11, 304
    movdqu %xmm12, 288(%rbp)
    .seh_savexmm %xmm12, 288
    movdqu %xmm13, 272(%rbp)
    .seh_savexmm %xmm13, 272
    movdqu %xmm14, 256(%rbp)
    .seh_savexmm %xmm14, 256
    movdqu %xmm15, 240(%rbp)
    .seh_savexmm %xmm15, 240
    movq %rbx, 224(%rbp)
    .seh_savereg %rbx, 224
    movq %r12, 216(%rbp)
    .seh_savereg %r12, 216
    movq %r13, 208(%rbp)
    .seh_savereg %r13, 208
    movq %r14, 200(%rbp)
    .seh_savereg %r14, 200
    movq %r15, 192(%rbp)
    .seh_savereg %r15, 192
    .seh_endprologue
    movq %rcx, 432(%rbp)
    movq %rdx, 440(%rbp)
    movq %r8, 448(%rbp)
    movq %r9, 456(%rbp)
    movq 440(%rbp), %rax
    movq %rax, 152(%rbp)
    movq 448(%rbp), %r10
    leaq 120(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    leaq 152(%rbp), %r10
    movq (%r10), %r12
    movq $168, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L4_4
    jmp .L4_26
.L4_26:
    movl %ebx, %r13d
    jmp .L4_5
.L4_4:
    leaq 120(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movl %ebx, %ebx
    movzbl %bl, %ebx
    movl $8, %ecx
    cmpl %ecx, %ebx
    setb %al
    movzbl %al, %r13d
.L4_5:
    movzbl %r13b, %ebx
    testl %ebx, %ebx
    jne .L4_1
    jmp .L4_2
.L4_1:
    leaq 160(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_27(%rip), %r12
    leaq 104(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $36, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L4_6:
    jmp .L4_3
.L4_2:
.L4_3:
    leaq 120(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %r13d
    movl %r13d, %r14d
    movzbl %r14b, %r14d
    movl $8, %ecx
    cmpl %ecx, %r14d
    jb .L4_8
.L4_7:
    movq $160, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movq $0, %rax
    movq %rbx, %r10
    movq %rax, (%r10)
    leaq 160(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movl $0, %eax
    movq %r13, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L4_10:
    jmp .L4_9
.L4_8:
.L4_9:
    movl $0, %ecx
    cmpl %ecx, %r13d
    jne .L4_12
.L4_11:
    movq %r12, %r10
    movzbl (%r10), %r13d
    movl $0, %ecx
    cmpl %ecx, %r13d
    jne .L4_15
.L4_14:
    leaq 160(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_28(%rip), %r12
    leaq 88(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $43, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L4_17:
    jmp .L4_16
.L4_15:
.L4_16:
    jmp .L4_13
.L4_12:
    movq %r12, %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L4_18
    jmp .L4_19
.L4_18:
    leaq 160(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_29(%rip), %r12
    leaq 72(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $46, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L4_21:
    jmp .L4_20
.L4_19:
.L4_20:
    movq $1, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movl $1, %ecx
    cmpl %ecx, %r13d
    sete %al
    movzbl %al, %r15d
    movq %r14, %r10
    movb %r15b, (%r10)
    movq $8, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq $0, %rax
    movq %r14, %r10
    movq %rax, (%r10)
    movq $24, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    subq $32, %rsp
    movq %r14, %rax
    movq %rax, 0(%rsp)
    movq 0(%rsp), %rcx
    call lb_utf8_Decoder_reset
    addq $32, %rsp
.L4_13:
    movq $8, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq (%r10), %r13
    movq $16, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq (%r10), %r14
    movq $8, %rcx
    movq %r12, %rax
    addq %rcx, %rax
    movq %rax, 48(%rbp)
    movq 48(%rbp), %r10
    movq (%r10), %r15
    movq %r14, %rax
    movq %r15, %rcx
    subq %rcx, %rax
    jnc 1f
    leaq .Ltext_30(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r14
    cmpq %r14, %r13
    jbe .L4_23
.L4_22:
    leaq 160(%rbp), %rbx
    leaq lb_net_15websocket_limit(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_31(%rip), %r12
    leaq 56(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $40, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L4_25:
    jmp .L4_24
.L4_23:
.L4_24:
    movq %r13, %rax
    movq %r15, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_32(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, 40(%rbp)
    movq 40(%rbp), %rax
    movq 48(%rbp), %r10
    movq %rax, (%r10)
    movq $1, %rcx
    movq %rbx, %rax
    addq %rcx, %rax
    movq %rax, 32(%rbp)
    movq 32(%rbp), %r10
    movzbl (%r10), %eax
    movl %eax, 24(%rbp)
    movl 24(%rbp), %eax
    movl $0, %ecx
    cmpl %ecx, %eax
    sete %al
    movzbl %al, %eax
    movl %eax, 16(%rbp)
    movl 16(%rbp), %eax
    movq %r12, %r10
    movb %al, (%r10)
    leaq 160(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %rax
    movq $24, %rcx
    movq %rax, %r14
    addq %rcx, %r14
    movl $0, %eax
    movq %r14, %r10
    movb %al, (%r10)
    movq 8(%rbp), %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketSequence_payload
lb_net_WebSocketSequence_payload:
    .seh_proc lb_net_WebSocketSequence_payload
    pushq %rbp
    .seh_pushreg %rbp
    subq $528, %rsp
    .seh_stackalloc 528
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 344(%rbp)
    movq %rdi, 520(%rbp)
    .seh_savereg %rdi, 520
    movq %rsi, 512(%rbp)
    .seh_savereg %rsi, 512
    movdqu %xmm6, 496(%rbp)
    .seh_savexmm %xmm6, 496
    movdqu %xmm7, 480(%rbp)
    .seh_savexmm %xmm7, 480
    movdqu %xmm8, 464(%rbp)
    .seh_savexmm %xmm8, 464
    movdqu %xmm9, 448(%rbp)
    .seh_savexmm %xmm9, 448
    movdqu %xmm10, 432(%rbp)
    .seh_savexmm %xmm10, 432
    movdqu %xmm11, 416(%rbp)
    .seh_savexmm %xmm11, 416
    movdqu %xmm12, 400(%rbp)
    .seh_savexmm %xmm12, 400
    movdqu %xmm13, 384(%rbp)
    .seh_savexmm %xmm13, 384
    movdqu %xmm14, 368(%rbp)
    .seh_savexmm %xmm14, 368
    movdqu %xmm15, 352(%rbp)
    .seh_savexmm %xmm15, 352
    movq %rbx, 336(%rbp)
    .seh_savereg %rbx, 336
    movq %r12, 328(%rbp)
    .seh_savereg %r12, 328
    movq %r13, 320(%rbp)
    .seh_savereg %r13, 320
    movq %r14, 312(%rbp)
    .seh_savereg %r14, 312
    movq %r15, 304(%rbp)
    .seh_savereg %r15, 304
    .seh_endprologue
    movq %rcx, 544(%rbp)
    movq %rdx, 552(%rbp)
    movq %r8, 560(%rbp)
    movq %r9, 568(%rbp)
    movq 552(%rbp), %rax
    movq %rax, 264(%rbp)
    movq 560(%rbp), %r10
    leaq 232(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 568(%rbp), %r10
    leaq 216(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 576(%rbp), %rax
    movl %eax, 200(%rbp)
    leaq 264(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 16(%rbp)
    leaq 232(%rbp), %rax
    movq %rax, 56(%rbp)
    movq 56(%rbp), %r10
    movzbl (%r10), %r12d
    movl %r12d, %r13d
    movzbl %r13b, %r13d
    movl $8, %ecx
    cmpl %ecx, %r13d
    jb .L5_2
.L5_1:
    movl $8, %ecx
    cmpl %ecx, %r12d
    jne .L5_5
.L5_4:
    movq 16(%rbp), %rax
    movq $32, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    movq 16(%rbp), %rax
    movq $160, %rcx
    addq %rcx, %rax
    movq %rax, 48(%rbp)
    movq 48(%rbp), %r10
    movq (%r10), %r15
    movq $126, %rcx
    cmpq %rcx, %r15
    jb 1f
    leaq .Ltext_33(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $125, %rcx
    cmpq %rcx, %r15
    jbe .L5_7
.L5_8:
    leaq .Ltext_33(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L5_7:
    movq %r13, %r12
    addq %r15, %r12
    movq $125, %rax
    subq %r15, %rax
    movq %rax, 40(%rbp)
    leaq 184(%rbp), %rbx
    movq %rbx, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq 40(%rbp), %rax
    movq %r12, %r10
    movq %rax, (%r10)
    leaq 216(%rbp), %r12
    movq $8, %rcx
    movq %r12, %rax
    addq %rcx, %rax
    movq %rax, 32(%rbp)
    movq 32(%rbp), %r10
    movq (%r10), %r14
    subq $64, %rsp
    movq %rbx, %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 0(%rsp)
    movq %r12, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 8(%rsp)
    movq %r14, %rax
    movq %rax, 16(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_memory_copy_0g1_u8
    addq $64, %rsp
    movq 48(%rbp), %r10
    movq (%r10), %rbx
    movq 32(%rbp), %r10
    movq (%r10), %r12
    movq %rbx, %rax
    movq %r12, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_34(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %rbx
    movq 48(%rbp), %r10
    movq %rbx, (%r10)
    leaq 200(%rbp), %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L5_9
    jmp .L5_10
.L5_9:
    movq $126, %rcx
    cmpq %rcx, %rbx
    jb 1f
    leaq .Ltext_35(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $0, %rax
    cmpq %rbx, %rax
    jbe .L5_12
.L5_13:
    leaq .Ltext_35(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L5_12:
    leaq 168(%rbp), %r12
    movq %r12, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq %rbx, (%r10)
    subq $48, %rsp
    movq %r12, %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    leaq 136(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_net_21websocket_check_close
    addq $48, %rsp
    leaq 136(%rbp), %r14
    movq $24, %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L5_15
    jmp .L5_14
.L5_15:
    leaq 272(%rbp), %rbx
    movq %r14, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 344(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 344(%rbp), %rax
    movq 520(%rbp), %rdi
    movq 512(%rbp), %rsi
    movdqu 496(%rbp), %xmm6
    movdqu 480(%rbp), %xmm7
    movdqu 464(%rbp), %xmm8
    movdqu 448(%rbp), %xmm9
    movdqu 432(%rbp), %xmm10
    movdqu 416(%rbp), %xmm11
    movdqu 400(%rbp), %xmm12
    movdqu 384(%rbp), %xmm13
    movdqu 368(%rbp), %xmm14
    movdqu 352(%rbp), %xmm15
    movq 336(%rbp), %rbx
    movq 328(%rbp), %r12
    movq 320(%rbp), %r13
    movq 312(%rbp), %r14
    movq 304(%rbp), %r15
    leaq 528(%rbp), %rsp
    popq %rbp
    ret
.L5_16:
.L5_14:
    movq 16(%rbp), %rax
    movq $168, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    jmp .L5_11
.L5_10:
.L5_11:
    jmp .L5_6
.L5_5:
.L5_6:
    leaq 272(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 344(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 344(%rbp), %rax
    movq 520(%rbp), %rdi
    movq 512(%rbp), %rsi
    movdqu 496(%rbp), %xmm6
    movdqu 480(%rbp), %xmm7
    movdqu 464(%rbp), %xmm8
    movdqu 448(%rbp), %xmm9
    movdqu 432(%rbp), %xmm10
    movdqu 416(%rbp), %xmm11
    movdqu 400(%rbp), %xmm12
    movdqu 384(%rbp), %xmm13
    movdqu 368(%rbp), %xmm14
    movdqu 352(%rbp), %xmm15
    movq 336(%rbp), %rbx
    movq 328(%rbp), %r12
    movq 320(%rbp), %r13
    movq 312(%rbp), %r14
    movq 304(%rbp), %r15
    leaq 528(%rbp), %rsp
    popq %rbp
    ret
.L5_17:
    jmp .L5_3
.L5_2:
.L5_3:
    movq 16(%rbp), %rax
    movq $1, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L5_18
    jmp .L5_19
.L5_18:
    leaq 216(%rbp), %rbx
    movq %rbx, %r10
    movq (%r10), %r12
    movq $8, %rcx
    addq %rcx, %rbx
    movq %rbx, %r10
    movq (%r10), %rbx
    movq 16(%rbp), %rax
    movq $24, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    leaq 96(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %rax
    movq $32, %rcx
    addq %rcx, %rax
    movq %rax, 24(%rbp)
    movq $0, %rax
    movq %rax, %r15
.L5_21:
    cmpq %rbx, %r15
    jae .L5_24
.L5_22:
    movq %r12, %r14
    addq %r15, %r14
    movq %r14, %r10
    movzbl (%r10), %r14d
    movzbl %r14b, %r14d
    subq $32, %rsp
    movq %r13, %rax
    movq %rax, 8(%rsp)
    movl %r14d, %eax
    movq %rax, 16(%rsp)
    leaq 96(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_utf8_Decoder_push
    addq $32, %rsp
    movq 24(%rbp), %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L5_26
    jmp .L5_25
.L5_26:
    movq 8(%rbp), %rax
    movq $8, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    leaq 272(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 344(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 344(%rbp), %rax
    movq 520(%rbp), %rdi
    movq 512(%rbp), %rsi
    movdqu 496(%rbp), %xmm6
    movdqu 480(%rbp), %xmm7
    movdqu 464(%rbp), %xmm8
    movdqu 448(%rbp), %xmm9
    movdqu 432(%rbp), %xmm10
    movdqu 416(%rbp), %xmm11
    movdqu 400(%rbp), %xmm12
    movdqu 384(%rbp), %xmm13
    movdqu 368(%rbp), %xmm14
    movdqu 352(%rbp), %xmm15
    movq 336(%rbp), %rbx
    movq 328(%rbp), %r12
    movq 320(%rbp), %r13
    movq 312(%rbp), %r14
    movq 304(%rbp), %r15
    leaq 528(%rbp), %rsp
    popq %rbp
    ret
.L5_27:
.L5_25:
.L5_23:
    movq $1, %rcx
    movq %r15, %r14
    addq %rcx, %r14
    movq %r14, %r15
    jmp .L5_21
.L5_24:
    leaq 200(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L5_31
    jmp .L5_36
.L5_36:
    movl %ebx, %r12d
    jmp .L5_32
.L5_31:
    movq 56(%rbp), %rax
    movq $1, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %r12d
.L5_32:
    movzbl %r12b, %ebx
    testl %ebx, %ebx
    jne .L5_28
    jmp .L5_29
.L5_28:
    movq 16(%rbp), %rax
    movq $24, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    subq $32, %rsp
    movq %rbx, %rax
    movq %rax, 8(%rsp)
    leaq 64(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_utf8_Decoder_finish
    addq $32, %rsp
    leaq 64(%rbp), %r12
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L5_34
    jmp .L5_33
.L5_34:
    leaq 272(%rbp), %rbx
    movq %r12, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movl $1, %eax
    movq %r13, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 344(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 344(%rbp), %rax
    movq 520(%rbp), %rdi
    movq 512(%rbp), %rsi
    movdqu 496(%rbp), %xmm6
    movdqu 480(%rbp), %xmm7
    movdqu 464(%rbp), %xmm8
    movdqu 448(%rbp), %xmm9
    movdqu 432(%rbp), %xmm10
    movdqu 416(%rbp), %xmm11
    movdqu 400(%rbp), %xmm12
    movdqu 384(%rbp), %xmm13
    movdqu 368(%rbp), %xmm14
    movdqu 352(%rbp), %xmm15
    movq 336(%rbp), %rbx
    movq 328(%rbp), %r12
    movq 320(%rbp), %r13
    movq 312(%rbp), %r14
    movq 304(%rbp), %r15
    leaq 528(%rbp), %rsp
    popq %rbp
    ret
.L5_35:
.L5_33:
    jmp .L5_30
.L5_29:
.L5_30:
    jmp .L5_20
.L5_19:
.L5_20:
    leaq 272(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 344(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 344(%rbp), %rax
    movq 520(%rbp), %rdi
    movq 512(%rbp), %rsi
    movdqu 496(%rbp), %xmm6
    movdqu 480(%rbp), %xmm7
    movdqu 464(%rbp), %xmm8
    movdqu 448(%rbp), %xmm9
    movdqu 432(%rbp), %xmm10
    movdqu 416(%rbp), %xmm11
    movdqu 400(%rbp), %xmm12
    movdqu 384(%rbp), %xmm13
    movdqu 368(%rbp), %xmm14
    movdqu 352(%rbp), %xmm15
    movq 336(%rbp), %rbx
    movq 328(%rbp), %r12
    movq 320(%rbp), %r13
    movq 312(%rbp), %r14
    movq 304(%rbp), %r15
    leaq 528(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketDecoder_create
lb_net_WebSocketDecoder_create:
    .seh_proc lb_net_WebSocketDecoder_create
    pushq %rbp
    .seh_pushreg %rbp
    subq $992, %rsp
    .seh_stackalloc 992
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 808(%rbp)
    movq %rdi, 984(%rbp)
    .seh_savereg %rdi, 984
    movq %rsi, 976(%rbp)
    .seh_savereg %rsi, 976
    movdqu %xmm6, 960(%rbp)
    .seh_savexmm %xmm6, 960
    movdqu %xmm7, 944(%rbp)
    .seh_savexmm %xmm7, 944
    movdqu %xmm8, 928(%rbp)
    .seh_savexmm %xmm8, 928
    movdqu %xmm9, 912(%rbp)
    .seh_savexmm %xmm9, 912
    movdqu %xmm10, 896(%rbp)
    .seh_savexmm %xmm10, 896
    movdqu %xmm11, 880(%rbp)
    .seh_savexmm %xmm11, 880
    movdqu %xmm12, 864(%rbp)
    .seh_savexmm %xmm12, 864
    movdqu %xmm13, 848(%rbp)
    .seh_savexmm %xmm13, 848
    movdqu %xmm14, 832(%rbp)
    .seh_savexmm %xmm14, 832
    movdqu %xmm15, 816(%rbp)
    .seh_savexmm %xmm15, 816
    movq %rbx, 800(%rbp)
    .seh_savereg %rbx, 800
    movq %r12, 792(%rbp)
    .seh_savereg %r12, 792
    movq %r13, 784(%rbp)
    .seh_savereg %r13, 784
    movq %r14, 776(%rbp)
    .seh_savereg %r14, 776
    .seh_endprologue
    movq %rcx, 1008(%rbp)
    movq %rdx, 1016(%rbp)
    movq %r8, 1024(%rbp)
    movq %r9, 1032(%rbp)
    movq 1016(%rbp), %rax
    movl %eax, 488(%rbp)
    movq 1024(%rbp), %rax
    movq %rax, 472(%rbp)
    movq 1032(%rbp), %rax
    movq %rax, 456(%rbp)
    leaq 184(%rbp), %rbx
    movq %rbx, %rdi
    xorl %esi, %esi
    movq $272, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memset
    addq $32, %rsp
    leaq 488(%rbp), %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movq %rbx, %r10
    movb %r12b, (%r10)
    leaq 472(%rbp), %r12
    movq %r12, %r10
    movq (%r10), %r12
    movq $8, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq %r12, (%r10)
    leaq 8(%rbp), %r12
    movq %r12, %rdi
    xorl %esi, %esi
    movq $176, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memset
    addq $32, %rsp
    leaq 456(%rbp), %r13
    movq %r13, %r10
    movq (%r10), %r13
    movq $16, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $88, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movq %r13, %rdi
    movq %r12, %rsi
    movq $176, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    leaq 504(%rbp), %r12
    movq %r12, %rdi
    movq %rbx, %rsi
    movq $272, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq %r12, %rsi
    movq 808(%rbp), %rdi
    movq $272, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 808(%rbp), %rax
    movq 984(%rbp), %rdi
    movq 976(%rbp), %rsi
    movdqu 960(%rbp), %xmm6
    movdqu 944(%rbp), %xmm7
    movdqu 928(%rbp), %xmm8
    movdqu 912(%rbp), %xmm9
    movdqu 896(%rbp), %xmm10
    movdqu 880(%rbp), %xmm11
    movdqu 864(%rbp), %xmm12
    movdqu 848(%rbp), %xmm13
    movdqu 832(%rbp), %xmm14
    movdqu 816(%rbp), %xmm15
    movq 800(%rbp), %rbx
    movq 792(%rbp), %r12
    movq 784(%rbp), %r13
    movq 776(%rbp), %r14
    leaq 992(%rbp), %rsp
    popq %rbp
    ret
.L6_1:
    leaq .Ltext_36(%rip), %rdi
    leaq .Ltext_19(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketDecoder_consume
lb_net_WebSocketDecoder_consume:
    .seh_proc lb_net_WebSocketDecoder_consume
    pushq %rbp
    .seh_pushreg %rbp
    subq $480, %rsp
    .seh_stackalloc 480
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 296(%rbp)
    movq %rdi, 472(%rbp)
    .seh_savereg %rdi, 472
    movq %rsi, 464(%rbp)
    .seh_savereg %rsi, 464
    movdqu %xmm6, 448(%rbp)
    .seh_savexmm %xmm6, 448
    movdqu %xmm7, 432(%rbp)
    .seh_savexmm %xmm7, 432
    movdqu %xmm8, 416(%rbp)
    .seh_savexmm %xmm8, 416
    movdqu %xmm9, 400(%rbp)
    .seh_savexmm %xmm9, 400
    movdqu %xmm10, 384(%rbp)
    .seh_savexmm %xmm10, 384
    movdqu %xmm11, 368(%rbp)
    .seh_savexmm %xmm11, 368
    movdqu %xmm12, 352(%rbp)
    .seh_savexmm %xmm12, 352
    movdqu %xmm13, 336(%rbp)
    .seh_savexmm %xmm13, 336
    movdqu %xmm14, 320(%rbp)
    .seh_savexmm %xmm14, 320
    movdqu %xmm15, 304(%rbp)
    .seh_savexmm %xmm15, 304
    movq %rbx, 288(%rbp)
    .seh_savereg %rbx, 288
    movq %r12, 280(%rbp)
    .seh_savereg %r12, 280
    movq %r13, 272(%rbp)
    .seh_savereg %r13, 272
    movq %r14, 264(%rbp)
    .seh_savereg %r14, 264
    movq %r15, 256(%rbp)
    .seh_savereg %r15, 256
    .seh_endprologue
    movq %rcx, 496(%rbp)
    movq %rdx, 504(%rbp)
    movq %r8, 512(%rbp)
    movq %r9, 520(%rbp)
    movq 504(%rbp), %rax
    movq %rax, 192(%rbp)
    movq 512(%rbp), %r10
    leaq 176(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 520(%rbp), %r10
    leaq 160(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 192(%rbp), %r10
    movq (%r10), %r13
    movq $264, %rcx
    movq %r13, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L7_1
    jmp .L7_2
.L7_1:
    leaq 200(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq $268, %rcx
    movq %r13, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movl (%r10), %r14d
    movq %r12, %r10
    movl %r14d, (%r10)
    leaq .Ltext_37(%rip), %r14
    leaq 144(%rbp), %r15
    movq %r15, %r10
    movq %r14, (%r10)
    movq $8, %rcx
    movq %r15, %r14
    addq %rcx, %r14
    movq $28, %rax
    movq %r14, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r15, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $48, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 296(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 296(%rbp), %rax
    movq 472(%rbp), %rdi
    movq 464(%rbp), %rsi
    movdqu 448(%rbp), %xmm6
    movdqu 432(%rbp), %xmm7
    movdqu 416(%rbp), %xmm8
    movdqu 400(%rbp), %xmm9
    movdqu 384(%rbp), %xmm10
    movdqu 368(%rbp), %xmm11
    movdqu 352(%rbp), %xmm12
    movdqu 336(%rbp), %xmm13
    movdqu 320(%rbp), %xmm14
    movdqu 304(%rbp), %xmm15
    movq 288(%rbp), %rbx
    movq 280(%rbp), %r12
    movq 272(%rbp), %r13
    movq 264(%rbp), %r14
    movq 256(%rbp), %r15
    leaq 480(%rbp), %rsp
    popq %rbp
    ret
.L7_4:
    jmp .L7_3
.L7_2:
.L7_3:
    leaq 160(%rbp), %r12
    movq $8, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq (%r10), %r14
    movq $0, %rcx
    cmpq %rcx, %r14
    jne .L7_6
.L7_5:
    leaq 200(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    leaq lb_io_14invalid_buffer(%rip), %r13
    movq %r13, %r10
    movl (%r10), %r13d
    movq %r12, %r10
    movl %r13d, (%r10)
    leaq .Ltext_38(%rip), %r13
    leaq 128(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $40, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $48, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 296(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 296(%rbp), %rax
    movq 472(%rbp), %rdi
    movq 464(%rbp), %rsi
    movdqu 448(%rbp), %xmm6
    movdqu 432(%rbp), %xmm7
    movdqu 416(%rbp), %xmm8
    movdqu 400(%rbp), %xmm9
    movdqu 384(%rbp), %xmm10
    movdqu 368(%rbp), %xmm11
    movdqu 352(%rbp), %xmm12
    movdqu 336(%rbp), %xmm13
    movdqu 320(%rbp), %xmm14
    movdqu 304(%rbp), %xmm15
    movq 288(%rbp), %rbx
    movq 280(%rbp), %r12
    movq 272(%rbp), %r13
    movq 264(%rbp), %r14
    movq 256(%rbp), %r15
    leaq 480(%rbp), %rsp
    popq %rbp
    ret
.L7_8:
    jmp .L7_7
.L7_6:
.L7_7:
    leaq 176(%rbp), %r14
    subq $64, %rsp
    movq %r13, %rax
    movq %rax, 8(%rsp)
    movq %r14, %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 16(%rsp)
    movq %r12, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 24(%rsp)
    leaq 24(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_net_WebSocketDecoder_advance
    addq $64, %rsp
    leaq 24(%rbp), %r15
    movq $48, %rcx
    movq %r15, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L7_12
    jmp .L7_11
.L7_12:
    movq $24, %rcx
    movq %r15, %r12
    addq %rcx, %r12
    leaq 80(%rbp), %r14
    movq %r12, %r10
    movq %r14, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    jmp .L7_9
.L7_11:
    leaq 104(%rbp), %rbx
    movq %r15, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    jmp .L7_10
.L7_9:
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $268, %rcx
    movq %r13, %rax
    addq %rcx, %rax
    movq %rax, 16(%rbp)
    movq %r14, %r10
    movl (%r10), %r15d
    movq 16(%rbp), %r10
    movl %r15d, (%r10)
    leaq 200(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %rax
    movq $24, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movl %r15d, (%r10)
    movq $8, %rcx
    movq %r14, %r15
    addq %rcx, %r15
    movq $8, %rcx
    addq %rcx, %r12
    movq %r15, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 8(%rbp), %rax
    movq $48, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq 8(%rbp), %rsi
    movq 296(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 296(%rbp), %rax
    movq 472(%rbp), %rdi
    movq 464(%rbp), %rsi
    movdqu 448(%rbp), %xmm6
    movdqu 432(%rbp), %xmm7
    movdqu 416(%rbp), %xmm8
    movdqu 400(%rbp), %xmm9
    movdqu 384(%rbp), %xmm10
    movdqu 368(%rbp), %xmm11
    movdqu 352(%rbp), %xmm12
    movdqu 336(%rbp), %xmm13
    movdqu 320(%rbp), %xmm14
    movdqu 304(%rbp), %xmm15
    movq 288(%rbp), %rbx
    movq 280(%rbp), %r12
    movq 272(%rbp), %r13
    movq 264(%rbp), %r14
    movq 256(%rbp), %r15
    leaq 480(%rbp), %rsp
    popq %rbp
    ret
.L7_13:
.L7_10:
    leaq 200(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movl $0, %eax
    movq %r13, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 296(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 296(%rbp), %rax
    movq 472(%rbp), %rdi
    movq 464(%rbp), %rsi
    movdqu 448(%rbp), %xmm6
    movdqu 432(%rbp), %xmm7
    movdqu 416(%rbp), %xmm8
    movdqu 400(%rbp), %xmm9
    movdqu 384(%rbp), %xmm10
    movdqu 368(%rbp), %xmm11
    movdqu 352(%rbp), %xmm12
    movdqu 336(%rbp), %xmm13
    movdqu 320(%rbp), %xmm14
    movdqu 304(%rbp), %xmm15
    movq 288(%rbp), %rbx
    movq 280(%rbp), %r12
    movq 272(%rbp), %r13
    movq 264(%rbp), %r14
    movq 256(%rbp), %r15
    leaq 480(%rbp), %rsp
    popq %rbp
    ret
.L7_14:
    leaq .Ltext_39(%rip), %rdi
    leaq .Ltext_19(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketDecoder_finish
lb_net_WebSocketDecoder_finish:
    .seh_proc lb_net_WebSocketDecoder_finish
    pushq %rbp
    .seh_pushreg %rbp
    subq $304, %rsp
    .seh_stackalloc 304
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 120(%rbp)
    movq %rdi, 296(%rbp)
    .seh_savereg %rdi, 296
    movq %rsi, 288(%rbp)
    .seh_savereg %rsi, 288
    movdqu %xmm6, 272(%rbp)
    .seh_savexmm %xmm6, 272
    movdqu %xmm7, 256(%rbp)
    .seh_savexmm %xmm7, 256
    movdqu %xmm8, 240(%rbp)
    .seh_savexmm %xmm8, 240
    movdqu %xmm9, 224(%rbp)
    .seh_savexmm %xmm9, 224
    movdqu %xmm10, 208(%rbp)
    .seh_savexmm %xmm10, 208
    movdqu %xmm11, 192(%rbp)
    .seh_savexmm %xmm11, 192
    movdqu %xmm12, 176(%rbp)
    .seh_savexmm %xmm12, 176
    movdqu %xmm13, 160(%rbp)
    .seh_savexmm %xmm13, 160
    movdqu %xmm14, 144(%rbp)
    .seh_savexmm %xmm14, 144
    movdqu %xmm15, 128(%rbp)
    .seh_savexmm %xmm15, 128
    movq %rbx, 112(%rbp)
    .seh_savereg %rbx, 112
    movq %r12, 104(%rbp)
    .seh_savereg %r12, 104
    movq %r13, 96(%rbp)
    .seh_savereg %r13, 96
    movq %r14, 88(%rbp)
    .seh_savereg %r14, 88
    movq %r15, 80(%rbp)
    .seh_savereg %r15, 80
    .seh_endprologue
    movq %rcx, 320(%rbp)
    movq %rdx, 328(%rbp)
    movq %r8, 336(%rbp)
    movq %r9, 344(%rbp)
    movq 328(%rbp), %rax
    movq %rax, 40(%rbp)
    leaq 40(%rbp), %r10
    movq (%r10), %r13
    movq $264, %rcx
    movq %r13, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L8_1
    jmp .L8_2
.L8_1:
    leaq 48(%rbp), %rbx
    movq $268, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_37(%rip), %r12
    leaq 24(%rbp), %r14
    movq %r14, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movq $28, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    movq 80(%rbp), %r15
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L8_4:
    jmp .L8_3
.L8_2:
.L8_3:
    movq $40, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L8_13
    jmp .L8_8
.L8_13:
    movl %r12d, %r14d
    jmp .L8_9
.L8_8:
    movq $32, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq (%r10), %r12
    movq $0, %rcx
    cmpq %rcx, %r12
    sete %al
    movzbl %al, %r12d
    movl $0, %ecx
    cmpl %ecx, %r12d
    sete %al
    movzbl %al, %r14d
.L8_9:
    movzbl %r14b, %r12d
    testl %r12d, %r12d
    jne .L8_14
    jmp .L8_10
.L8_14:
    movl %r12d, %r14d
    jmp .L8_11
.L8_10:
    movq $88, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $168, %rcx
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movl $0, %ecx
    cmpl %ecx, %r12d
    sete %al
    movzbl %al, %r14d
.L8_11:
    movzbl %r14b, %r12d
    testl %r12d, %r12d
    jne .L8_5
    jmp .L8_6
.L8_5:
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq $268, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    leaq lb_io_14unexpected_eof(%rip), %r14
    movq %r14, %r10
    movl (%r10), %r15d
    movq %r12, %r10
    movl %r15d, (%r10)
    leaq 48(%rbp), %r12
    movq %r14, %r10
    movl (%r10), %r14d
    movq %r12, %r10
    movl %r14d, (%r10)
    leaq .Ltext_40(%rip), %r14
    leaq 8(%rbp), %r15
    movq %r15, %r10
    movq %r14, (%r10)
    movq $8, %rcx
    movq %r15, %r14
    addq %rcx, %r14
    movq $46, %rax
    movq %r14, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r15, %r10
    movq %r14, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movl $1, %eax
    movq %r14, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 120(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    movq 80(%rbp), %r15
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
.L8_12:
    jmp .L8_7
.L8_6:
.L8_7:
    leaq 48(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 120(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 120(%rbp), %rax
    movq 296(%rbp), %rdi
    movq 288(%rbp), %rsi
    movdqu 272(%rbp), %xmm6
    movdqu 256(%rbp), %xmm7
    movdqu 240(%rbp), %xmm8
    movdqu 224(%rbp), %xmm9
    movdqu 208(%rbp), %xmm10
    movdqu 192(%rbp), %xmm11
    movdqu 176(%rbp), %xmm12
    movdqu 160(%rbp), %xmm13
    movdqu 144(%rbp), %xmm14
    movdqu 128(%rbp), %xmm15
    movq 112(%rbp), %rbx
    movq 104(%rbp), %r12
    movq 96(%rbp), %r13
    movq 88(%rbp), %r14
    movq 80(%rbp), %r15
    leaq 304(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketDecoder_advance
lb_net_WebSocketDecoder_advance:
    .seh_proc lb_net_WebSocketDecoder_advance
    pushq %rbp
    .seh_pushreg %rbp
    subq $848, %rsp
    .seh_stackalloc 848
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 664(%rbp)
    movq %rdi, 840(%rbp)
    .seh_savereg %rdi, 840
    movq %rsi, 832(%rbp)
    .seh_savereg %rsi, 832
    movdqu %xmm6, 816(%rbp)
    .seh_savexmm %xmm6, 816
    movdqu %xmm7, 800(%rbp)
    .seh_savexmm %xmm7, 800
    movdqu %xmm8, 784(%rbp)
    .seh_savexmm %xmm8, 784
    movdqu %xmm9, 768(%rbp)
    .seh_savexmm %xmm9, 768
    movdqu %xmm10, 752(%rbp)
    .seh_savexmm %xmm10, 752
    movdqu %xmm11, 736(%rbp)
    .seh_savexmm %xmm11, 736
    movdqu %xmm12, 720(%rbp)
    .seh_savexmm %xmm12, 720
    movdqu %xmm13, 704(%rbp)
    .seh_savexmm %xmm13, 704
    movdqu %xmm14, 688(%rbp)
    .seh_savexmm %xmm14, 688
    movdqu %xmm15, 672(%rbp)
    .seh_savexmm %xmm15, 672
    movq %rbx, 656(%rbp)
    .seh_savereg %rbx, 656
    movq %r12, 648(%rbp)
    .seh_savereg %r12, 648
    movq %r13, 640(%rbp)
    .seh_savereg %r13, 640
    movq %r14, 632(%rbp)
    .seh_savereg %r14, 632
    movq %r15, 624(%rbp)
    .seh_savereg %r15, 624
    .seh_endprologue
    movq %rcx, 864(%rbp)
    movq %rdx, 872(%rbp)
    movq %r8, 880(%rbp)
    movq %r9, 888(%rbp)
    movq 872(%rbp), %rax
    movq %rax, 560(%rbp)
    movq 880(%rbp), %r10
    leaq 544(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 888(%rbp), %r10
    leaq 528(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 560(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 48(%rbp)
    movq 48(%rbp), %rax
    movq $40, %rcx
    addq %rcx, %rax
    movq %rax, 272(%rbp)
    leaq 544(%rbp), %r12
    movq $8, %rcx
    movq %r12, %rax
    addq %rcx, %rax
    movq %rax, 264(%rbp)
    movq 48(%rbp), %rax
    movq $32, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    movq 48(%rbp), %rax
    movq $16, %rcx
    addq %rcx, %rax
    movq %rax, 256(%rbp)
    leaq 544(%rbp), %rax
    movq %rax, 248(%rbp)
    movq 248(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 240(%rbp)
    leaq 512(%rbp), %rax
    movq %rax, 232(%rbp)
    movq 232(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 224(%rbp)
    movq 48(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 216(%rbp)
    leaq 440(%rbp), %rax
    movq %rax, 16(%rbp)
    movq 16(%rbp), %rax
    movq $64, %rcx
    addq %rcx, %rax
    movq %rax, 208(%rbp)
    movq 16(%rbp), %rax
    movq $32, %rcx
    addq %rcx, %rax
    movq %rax, 200(%rbp)
    leaq 408(%rbp), %rax
    movq %rax, 192(%rbp)
    movq 48(%rbp), %rax
    movq $88, %rcx
    addq %rcx, %rax
    movq %rax, 184(%rbp)
    leaq 376(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %rax
    movq $24, %rcx
    addq %rcx, %rax
    movq %rax, 176(%rbp)
    movq 48(%rbp), %rax
    movq $48, %rcx
    addq %rcx, %rax
    movq %rax, 168(%rbp)
    movq 48(%rbp), %rax
    movq $80, %rcx
    addq %rcx, %rax
    movq %rax, 160(%rbp)
    movq $0, %rax
    movq %rax, 40(%rbp)
    movl $0, %eax
    movl %eax, 32(%rbp)
.L9_1:
    movq 272(%rbp), %r10
    movzbl (%r10), %r15d
    movl $0, %ecx
    cmpl %ecx, %r15d
    sete %al
    movzbl %al, %r15d
    testl %r15d, %r15d
    jne .L9_4
    jmp .L9_50
.L9_50:
    movl %r15d, %ebx
    jmp .L9_5
.L9_4:
    movq 264(%rbp), %r10
    movq (%r10), %rbx
    movq 40(%rbp), %rax
    cmpq %rbx, %rax
    setb %al
    movzbl %al, %ebx
.L9_5:
    movzbl %bl, %r12d
    testl %r12d, %r12d
    jne .L9_2
    jmp .L9_3
.L9_2:
    movq %r13, %r10
    movq (%r10), %rbx
    movq $14, %rcx
    cmpq %rcx, %rbx
    jb 1f
    leaq .Ltext_41(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 256(%rbp), %rcx
    addq %rcx, %rbx
    movq 248(%rbp), %r10
    movq (%r10), %r12
    movq 240(%rbp), %r10
    movq (%r10), %r15
    movq 40(%rbp), %rax
    cmpq %r15, %rax
    jb 1f
    leaq .Ltext_41(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 40(%rbp), %rcx
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movq %rbx, %r10
    movb %r12b, (%r10)
    movq %r13, %r10
    movq (%r10), %rbx
    movq %rbx, %rax
    movq $1, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_42(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %rbx
    movq %r13, %r10
    movq %rbx, (%r10)
    movq 40(%rbp), %rax
    movq $1, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movq $15, %rcx
    cmpq %rcx, %rbx
    jb 1f
    leaq .Ltext_44(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $0, %rax
    cmpq %rbx, %rax
    jbe .L9_9
.L9_10:
    leaq .Ltext_44(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L9_9:
    movq 256(%rbp), %rax
    movq 232(%rbp), %r10
    movq %rax, (%r10)
    movq 224(%rbp), %r10
    movq %rbx, (%r10)
    movq 48(%rbp), %r10
    movzbl (%r10), %r14d
    movq 216(%rbp), %r10
    movq (%r10), %r15
    subq $48, %rsp
    movq 232(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    movl %r14d, %eax
    movq %rax, 16(%rsp)
    movq %r15, %rax
    movq %rax, 24(%rsp)
    leaq 440(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_net_21websocket_parse_frame
    addq $48, %rsp
    movq 208(%rbp), %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L9_12
    jmp .L9_11
.L9_12:
    movq 16(%rbp), %rax
    movq $40, %rcx
    movq %rax, %rbx
    addq %rcx, %rbx
    leaq 568(%rbp), %r12
    movq $24, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movq %rbx, %r10
    movq %r13, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 664(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    movq 624(%rbp), %r15
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
.L9_11:
    movq 200(%rbp), %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L9_14
    jmp .L9_7
.L9_14:
    movq 16(%rbp), %r10
    movq 192(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
.L9_6:
    subq $64, %rsp
    movq 184(%rbp), %rax
    movq %rax, 8(%rsp)
    movq 192(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 16(%rsp)
    leaq 376(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_net_WebSocketSequence_begin
    addq $64, %rsp
    movq 176(%rbp), %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L9_16
    jmp .L9_15
.L9_16:
    leaq 568(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq 8(%rbp), %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 664(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    movq 624(%rbp), %r15
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
.L9_15:
    movq 192(%rbp), %r10
    movq 168(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movl $1, %eax
    movq 272(%rbp), %r10
    movb %al, (%r10)
    movq $0, %rax
    movq 160(%rbp), %r10
    movq %rax, (%r10)
    movq $0, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    movl $1, %eax
    movl %eax, %ebx
    jmp .L9_8
.L9_7:
    movl 32(%rbp), %eax
    movl %eax, %ebx
.L9_8:
    movq %r12, %rax
    movq %rax, 40(%rbp)
    movl %ebx, %eax
    movl %eax, 32(%rbp)
    jmp .L9_1
.L9_3:
    testl %r15d, %r15d
    jne .L9_18
    jmp .L9_19
.L9_18:
    leaq 352(%rbp), %rbx
    movq %rbx, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movq $0, 16(%r11)
    movq 40(%rbp), %rax
    movq %rbx, %r10
    movq %rax, (%r10)
    leaq 568(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $0, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 664(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    movq 624(%rbp), %r15
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
.L9_19:
.L9_20:
    leaq 544(%rbp), %rbx
    movq $8, %rcx
    movq %rbx, %rax
    addq %rcx, %rax
    movq %rax, 152(%rbp)
    movq 152(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 144(%rbp)
    movq 144(%rbp), %rax
    movq 40(%rbp), %rcx
    subq %rcx, %rax
    jnc 1f
    leaq .Ltext_45(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r13
    leaq 528(%rbp), %rax
    movq %rax, 136(%rbp)
    movq 136(%rbp), %rax
    movq $8, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq (%r10), %rax
    movq %rax, 128(%rbp)
    movq 128(%rbp), %rcx
    cmpq %rcx, %r13
    jbe .L9_23
.L9_22:
    movq 128(%rbp), %rax
    movq %rax, %r13
    jmp .L9_24
.L9_23:
.L9_24:
    movq 48(%rbp), %rax
    movq $48, %rcx
    addq %rcx, %rax
    movq %rax, 120(%rbp)
    movq 120(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 112(%rbp)
    movq 112(%rbp), %r10
    movq (%r10), %r12
    movq 48(%rbp), %rax
    movq $80, %rcx
    addq %rcx, %rax
    movq %rax, 104(%rbp)
    movq 104(%rbp), %r10
    movq (%r10), %r14
    movq %r12, %rax
    movq %r14, %rcx
    subq %rcx, %rax
    jnc 1f
    leaq .Ltext_46(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r12
    cmpq %r12, %r13
    jbe .L9_26
.L9_25:
    movq %r12, %rax
    movq %rax, 24(%rbp)
    jmp .L9_27
.L9_26:
    movq %r13, %rax
    movq %rax, 24(%rbp)
.L9_27:
    movq %rbx, %r10
    movq (%r10), %rax
    movq %rax, 96(%rbp)
    movq 136(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 88(%rbp)
    movq 120(%rbp), %rax
    movq $28, %rcx
    addq %rcx, %rax
    movq %rax, 80(%rbp)
    movq 120(%rbp), %rax
    movq $24, %rcx
    addq %rcx, %rax
    movq %rax, 72(%rbp)
    movq $0, %rax
    movq %rax, %r14
.L9_28:
    movq 24(%rbp), %rcx
    cmpq %rcx, %r14
    jae .L9_31
.L9_29:
    movq 40(%rbp), %rax
    movq %r14, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_48(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %rbx
    movq 144(%rbp), %rcx
    cmpq %rcx, %rbx
    jb 1f
    leaq .Ltext_48(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 96(%rbp), %rcx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movq 128(%rbp), %rcx
    cmpq %rcx, %r14
    jb 1f
    leaq .Ltext_49(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 88(%rbp), %rax
    movq %rax, %r13
    addq %r14, %r13
    movq 80(%rbp), %r10
    movzbl (%r10), %r15d
    testl %r15d, %r15d
    jne .L9_32
    jmp .L9_33
.L9_32:
    movzbl %bl, %r12d
    movq 104(%rbp), %r10
    movq (%r10), %r15
    movq %r15, %rax
    movq %r14, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_49(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r15
.L9_36:
    movq $4, %rcx
    movq %r15, %rax
    xorl %edx, %edx
    divq %rcx
    movq %rdx, %rbx
    movq $4, %rcx
    cmpq %rcx, %rbx
    jb 1f
    leaq .Ltext_49(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 72(%rbp), %rcx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    xorl %r12d, %ebx
    jmp .L9_34
.L9_33:
    movzbl %bl, %r12d
    movl %r12d, %ebx
.L9_34:
    movzbl %bl, %r12d
    movq %r13, %r10
    movb %r12b, (%r10)
.L9_30:
    movq $1, %rcx
    movq %r14, %rbx
    addq %rcx, %rbx
    movq %rbx, %r14
    jmp .L9_28
.L9_31:
    movq 104(%rbp), %r10
    movq (%r10), %rbx
    movq %rbx, %rax
    movq 24(%rbp), %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_51(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %rbx
    movq 104(%rbp), %r10
    movq %rbx, (%r10)
    movq 112(%rbp), %r10
    movq (%r10), %r12
    cmpq %r12, %rbx
    sete %al
    movzbl %al, %ebx
    movq 48(%rbp), %rax
    movq $88, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movq 136(%rbp), %r10
    movq (%r10), %r14
    movq 128(%rbp), %rax
    movq $1, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    movq $0, %rax
    cmpq %r13, %rax
    jb 1f
    leaq .Ltext_52(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 24(%rbp), %rax
    cmpq %r13, %rax
    jb 1f
    leaq .Ltext_52(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $0, %rax
    movq 24(%rbp), %rcx
    cmpq %rcx, %rax
    jbe .L9_37
.L9_38:
    leaq .Ltext_52(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L9_37:
    leaq 336(%rbp), %r13
    movq %r13, %r10
    movq %r14, (%r10)
    movq $8, %rcx
    movq %r13, %rax
    addq %rcx, %rax
    movq %rax, 64(%rbp)
    movq 24(%rbp), %rax
    movq 64(%rbp), %r10
    movq %rax, (%r10)
    movzbl %bl, %eax
    movl %eax, 56(%rbp)
    subq $96, %rsp
    movq %r12, %rax
    movq %rax, 8(%rsp)
    movq 120(%rbp), %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movq %r13, %r10
    leaq 80(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 80(%rsp), %rax
    movq %rax, 24(%rsp)
    movl 56(%rbp), %eax
    movq %rax, 32(%rsp)
    leaq 304(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_net_WebSocketSequence_payload
    addq $96, %rsp
    leaq 304(%rbp), %r15
    movq $24, %rcx
    movq %r15, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movzbl (%r10), %r13d
    testl %r13d, %r13d
    jne .L9_40
    jmp .L9_39
.L9_40:
    leaq 568(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r15, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 664(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    movq 624(%rbp), %r15
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
.L9_39:
    movl 56(%rbp), %eax
    testl %eax, %eax
    jne .L9_42
    jmp .L9_43
.L9_42:
    movl $0, %eax
    movq 272(%rbp), %r10
    movb %al, (%r10)
    jmp .L9_44
.L9_43:
.L9_44:
    leaq 280(%rbp), %r14
    movq %r14, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movq $0, 16(%r11)
    movq 40(%rbp), %rax
    movq 24(%rbp), %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_53(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %rbx
    movq %r14, %r10
    movq %rbx, (%r10)
    movq $8, %rcx
    movq %r14, %rbx
    addq %rcx, %rbx
    movq 24(%rbp), %rax
    movq %rbx, %r10
    movq %rax, (%r10)
    movq 120(%rbp), %r10
    movzbl (%r10), %ebx
    movq $16, %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movb %bl, (%r10)
    movl 32(%rbp), %eax
    movzbl %al, %r12d
    movq $17, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movb %r12b, (%r10)
    movq $18, %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movl 56(%rbp), %eax
    movq %r12, %r10
    movb %al, (%r10)
    movl 56(%rbp), %eax
    testl %eax, %eax
    jne .L9_45
    jmp .L9_51
.L9_51:
    movl 56(%rbp), %eax
    movl %eax, %r12d
    jmp .L9_46
.L9_45:
    movq 120(%rbp), %rax
    movq $1, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
.L9_46:
    movzbl %r12b, %r13d
    testl %r13d, %r13d
    jne .L9_47
    jmp .L9_52
.L9_52:
    movl %r13d, %ebx
    jmp .L9_48
.L9_47:
    movl %ebx, %r12d
    movzbl %r12b, %r12d
    movl $8, %ecx
    cmpl %ecx, %r12d
    setb %al
    movzbl %al, %r12d
    movl %r12d, %ebx
.L9_48:
    movzbl %bl, %r12d
    movq $19, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movb %r12b, (%r10)
    leaq 568(%rbp), %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $48, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movl $0, %eax
    movq %r13, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 664(%rbp), %rdi
    movq $56, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    movq 624(%rbp), %r15
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketEncoder_create
lb_net_WebSocketEncoder_create:
    .seh_proc lb_net_WebSocketEncoder_create
    pushq %rbp
    .seh_pushreg %rbp
    subq $848, %rsp
    .seh_stackalloc 848
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 664(%rbp)
    movq %rdi, 840(%rbp)
    .seh_savereg %rdi, 840
    movq %rsi, 832(%rbp)
    .seh_savereg %rsi, 832
    movdqu %xmm6, 816(%rbp)
    .seh_savexmm %xmm6, 816
    movdqu %xmm7, 800(%rbp)
    .seh_savexmm %xmm7, 800
    movdqu %xmm8, 784(%rbp)
    .seh_savexmm %xmm8, 784
    movdqu %xmm9, 768(%rbp)
    .seh_savexmm %xmm9, 768
    movdqu %xmm10, 752(%rbp)
    .seh_savexmm %xmm10, 752
    movdqu %xmm11, 736(%rbp)
    .seh_savexmm %xmm11, 736
    movdqu %xmm12, 720(%rbp)
    .seh_savexmm %xmm12, 720
    movdqu %xmm13, 704(%rbp)
    .seh_savexmm %xmm13, 704
    movdqu %xmm14, 688(%rbp)
    .seh_savexmm %xmm14, 688
    movdqu %xmm15, 672(%rbp)
    .seh_savexmm %xmm15, 672
    movq %rbx, 656(%rbp)
    .seh_savereg %rbx, 656
    movq %r12, 648(%rbp)
    .seh_savereg %r12, 648
    movq %r13, 640(%rbp)
    .seh_savereg %r13, 640
    movq %r14, 632(%rbp)
    .seh_savereg %r14, 632
    .seh_endprologue
    movq %rcx, 864(%rbp)
    movq %rdx, 872(%rbp)
    movq %r8, 880(%rbp)
    movq %r9, 888(%rbp)
    movq 872(%rbp), %rax
    movl %eax, 416(%rbp)
    movq 880(%rbp), %rax
    movq %rax, 400(%rbp)
    movq 888(%rbp), %rax
    movq %rax, 384(%rbp)
    leaq 184(%rbp), %rbx
    movq %rbx, %rdi
    xorl %esi, %esi
    movq $200, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memset
    addq $32, %rsp
    leaq 416(%rbp), %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movq %rbx, %r10
    movb %r12b, (%r10)
    leaq 400(%rbp), %r12
    movq %r12, %r10
    movq (%r10), %r12
    movq $8, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq %r12, (%r10)
    leaq 8(%rbp), %r12
    movq %r12, %rdi
    xorl %esi, %esi
    movq $176, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memset
    addq $32, %rsp
    leaq 384(%rbp), %r13
    movq %r13, %r10
    movq (%r10), %r13
    movq $16, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $16, %rcx
    movq %rbx, %r13
    addq %rcx, %r13
    movq %r13, %rdi
    movq %r12, %rsi
    movq $176, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    leaq 432(%rbp), %r12
    movq %r12, %rdi
    movq %rbx, %rsi
    movq $200, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq %r12, %rsi
    movq 664(%rbp), %rdi
    movq $200, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 664(%rbp), %rax
    movq 840(%rbp), %rdi
    movq 832(%rbp), %rsi
    movdqu 816(%rbp), %xmm6
    movdqu 800(%rbp), %xmm7
    movdqu 784(%rbp), %xmm8
    movdqu 768(%rbp), %xmm9
    movdqu 752(%rbp), %xmm10
    movdqu 736(%rbp), %xmm11
    movdqu 720(%rbp), %xmm12
    movdqu 704(%rbp), %xmm13
    movdqu 688(%rbp), %xmm14
    movdqu 672(%rbp), %xmm15
    movq 656(%rbp), %rbx
    movq 648(%rbp), %r12
    movq 640(%rbp), %r13
    movq 632(%rbp), %r14
    leaq 848(%rbp), %rsp
    popq %rbp
    ret
.L10_1:
    leaq .Ltext_54(%rip), %rdi
    leaq .Ltext_19(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketEncoder_write
lb_net_WebSocketEncoder_write:
    .seh_proc lb_net_WebSocketEncoder_write
    pushq %rbp
    .seh_pushreg %rbp
    subq $416, %rsp
    .seh_stackalloc 416
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 232(%rbp)
    movq %rdi, 408(%rbp)
    .seh_savereg %rdi, 408
    movq %rsi, 400(%rbp)
    .seh_savereg %rsi, 400
    movdqu %xmm6, 384(%rbp)
    .seh_savexmm %xmm6, 384
    movdqu %xmm7, 368(%rbp)
    .seh_savexmm %xmm7, 368
    movdqu %xmm8, 352(%rbp)
    .seh_savexmm %xmm8, 352
    movdqu %xmm9, 336(%rbp)
    .seh_savexmm %xmm9, 336
    movdqu %xmm10, 320(%rbp)
    .seh_savexmm %xmm10, 320
    movdqu %xmm11, 304(%rbp)
    .seh_savexmm %xmm11, 304
    movdqu %xmm12, 288(%rbp)
    .seh_savexmm %xmm12, 288
    movdqu %xmm13, 272(%rbp)
    .seh_savexmm %xmm13, 272
    movdqu %xmm14, 256(%rbp)
    .seh_savexmm %xmm14, 256
    movdqu %xmm15, 240(%rbp)
    .seh_savexmm %xmm15, 240
    movq %rbx, 224(%rbp)
    .seh_savereg %rbx, 224
    movq %r12, 216(%rbp)
    .seh_savereg %r12, 216
    movq %r13, 208(%rbp)
    .seh_savereg %r13, 208
    movq %r14, 200(%rbp)
    .seh_savereg %r14, 200
    movq %r15, 192(%rbp)
    .seh_savereg %r15, 192
    .seh_endprologue
    movq %rcx, 432(%rbp)
    movq %rdx, 440(%rbp)
    movq %r8, 448(%rbp)
    movq %r9, 456(%rbp)
    movq 440(%rbp), %rax
    movq %rax, 152(%rbp)
    movq 448(%rbp), %r10
    leaq 136(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 456(%rbp), %rax
    movl %eax, 120(%rbp)
    movq 464(%rbp), %r10
    leaq 104(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 472(%rbp), %rax
    movl %eax, 88(%rbp)
    leaq 152(%rbp), %r10
    movq (%r10), %r13
    movq $192, %rcx
    movq %r13, %rax
    addq %rcx, %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L11_1
    jmp .L11_2
.L11_1:
    leaq 160(%rbp), %rbx
    movq $196, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_55(%rip), %r12
    leaq 72(%rbp), %r14
    movq %r14, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movq $28, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r14, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L11_4:
    jmp .L11_3
.L11_2:
.L11_3:
    leaq 136(%rbp), %r12
    leaq 120(%rbp), %r14
    movq %r14, %r10
    movzbl (%r10), %r14d
    leaq 104(%rbp), %r15
    leaq 88(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    subq $80, %rsp
    movq %r13, %rax
    movq %rax, 8(%rsp)
    movq %r12, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movl %r14d, %eax
    movq %rax, 24(%rsp)
    movq %r15, %r10
    leaq 64(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 64(%rsp), %rax
    movq %rax, 32(%rsp)
    movl %ebx, %eax
    movq %rax, 40(%rsp)
    leaq 16(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_net_WebSocketEncoder_emit
    addq $80, %rsp
    leaq 16(%rbp), %r12
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L11_8
    jmp .L11_7
.L11_8:
    leaq 48(%rbp), %rbx
    movq %r12, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    jmp .L11_5
.L11_7:
    jmp .L11_6
.L11_5:
    movl $1, %eax
    movq 8(%rbp), %r10
    movb %al, (%r10)
    movq $196, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %rbx, %r10
    movl (%r10), %r14d
    movq %r12, %r10
    movl %r14d, (%r10)
    leaq 160(%rbp), %r12
    movq %r12, %r10
    movl %r14d, (%r10)
    movq $8, %rcx
    movq %rbx, %r14
    addq %rcx, %r14
    movq $8, %rcx
    movq %r12, %r15
    addq %rcx, %r15
    movq %r14, %r10
    movq %r15, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %r12, %r14
    addq %rcx, %r14
    movl $1, %eax
    movq %r14, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
.L11_9:
.L11_6:
    leaq 160(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 232(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 232(%rbp), %rax
    movq 408(%rbp), %rdi
    movq 400(%rbp), %rsi
    movdqu 384(%rbp), %xmm6
    movdqu 368(%rbp), %xmm7
    movdqu 352(%rbp), %xmm8
    movdqu 336(%rbp), %xmm9
    movdqu 320(%rbp), %xmm10
    movdqu 304(%rbp), %xmm11
    movdqu 288(%rbp), %xmm12
    movdqu 272(%rbp), %xmm13
    movdqu 256(%rbp), %xmm14
    movdqu 240(%rbp), %xmm15
    movq 224(%rbp), %rbx
    movq 216(%rbp), %r12
    movq 208(%rbp), %r13
    movq 200(%rbp), %r14
    movq 192(%rbp), %r15
    leaq 416(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .p2align 4
    .globl lb_net_WebSocketEncoder_emit
lb_net_WebSocketEncoder_emit:
    .seh_proc lb_net_WebSocketEncoder_emit
    pushq %rbp
    .seh_pushreg %rbp
    movq $4976, %rax
    call ___chkstk_ms
    subq $4976, %rsp
    .seh_stackalloc 4976
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rcx, 4792(%rbp)
    movq %rdi, 4968(%rbp)
    .seh_savereg %rdi, 4968
    movq %rsi, 4960(%rbp)
    .seh_savereg %rsi, 4960
    movdqu %xmm6, 4944(%rbp)
    .seh_savexmm %xmm6, 4944
    movdqu %xmm7, 4928(%rbp)
    .seh_savexmm %xmm7, 4928
    movdqu %xmm8, 4912(%rbp)
    .seh_savexmm %xmm8, 4912
    movdqu %xmm9, 4896(%rbp)
    .seh_savexmm %xmm9, 4896
    movdqu %xmm10, 4880(%rbp)
    .seh_savexmm %xmm10, 4880
    movdqu %xmm11, 4864(%rbp)
    .seh_savexmm %xmm11, 4864
    movdqu %xmm12, 4848(%rbp)
    .seh_savexmm %xmm12, 4848
    movdqu %xmm13, 4832(%rbp)
    .seh_savexmm %xmm13, 4832
    movdqu %xmm14, 4816(%rbp)
    .seh_savexmm %xmm14, 4816
    movdqu %xmm15, 4800(%rbp)
    .seh_savexmm %xmm15, 4800
    movq %rbx, 4784(%rbp)
    .seh_savereg %rbx, 4784
    movq %r12, 4776(%rbp)
    .seh_savereg %r12, 4776
    movq %r13, 4768(%rbp)
    .seh_savereg %r13, 4768
    movq %r14, 4760(%rbp)
    .seh_savereg %r14, 4760
    movq %r15, 4752(%rbp)
    .seh_savereg %r15, 4752
    .seh_endprologue
    movq %rcx, 4992(%rbp)
    movq %rdx, 5000(%rbp)
    movq %r8, 5008(%rbp)
    movq %r9, 5016(%rbp)
    movq 5000(%rbp), %rax
    movq %rax, 4712(%rbp)
    movq 5008(%rbp), %r10
    leaq 4696(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 5016(%rbp), %rax
    movl %eax, 4680(%rbp)
    movq 5024(%rbp), %r10
    leaq 4664(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 5032(%rbp), %rax
    movl %eax, 4648(%rbp)
    leaq 4712(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 48(%rbp)
    leaq 4680(%rbp), %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    movl %ebx, %r12d
    movzbl %r12b, %eax
    movl %eax, 144(%rbp)
    subq $32, %rsp
    movl 144(%rbp), %eax
    movq %rax, 8(%rsp)
    leaq 4608(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_net_16websocket_opcode
    addq $32, %rsp
    leaq 4608(%rbp), %r15
    movq $32, %rcx
    movq %r15, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movzbl (%r10), %r13d
    testl %r13d, %r13d
    jne .L12_2
    jmp .L12_1
.L12_2:
    movq $8, %rcx
    movq %r15, %rbx
    addq %rcx, %rbx
    leaq 4720(%rbp), %r12
    movq %rbx, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %r12, %rbx
    addq %rcx, %rbx
    movl $1, %eax
    movq %rbx, %r10
    movb %al, (%r10)
    movq %r12, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_1:
    leaq 4664(%rbp), %rax
    movq %rax, 136(%rbp)
    movq 136(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 128(%rbp)
    movq 128(%rbp), %r10
    movq (%r10), %r15
    movq 48(%rbp), %rax
    movq $8, %rcx
    movq %rax, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq (%r10), %r13
    cmpq %r13, %r15
    jbe .L12_5
.L12_4:
    leaq 4720(%rbp), %rbx
    leaq lb_net_15websocket_limit(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_13(%rip), %r12
    leaq 4592(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $38, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_5:
.L12_6:
    movl 144(%rbp), %eax
    movl $8, %ecx
    cmpl %ecx, %eax
    setae %al
    movzbl %al, %r13d
    testl %r13d, %r13d
    jne .L12_11
    jmp .L12_77
.L12_77:
    movl %r13d, %r12d
    jmp .L12_12
.L12_11:
    leaq 4648(%rbp), %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
    movl $0, %ecx
    cmpl %ecx, %r12d
    sete %al
    movzbl %al, %r12d
    testl %r12d, %r12d
    jne .L12_78
    jmp .L12_13
.L12_78:
    movl %r12d, %r13d
    jmp .L12_14
.L12_13:
    movq $125, %rcx
    cmpq %rcx, %r15
    seta %al
    movzbl %al, %r13d
.L12_14:
    movzbl %r13b, %r12d
.L12_12:
    movzbl %r12b, %r13d
    testl %r13d, %r13d
    jne .L12_8
    jmp .L12_9
.L12_8:
    leaq 4720(%rbp), %rbx
    leaq lb_net_17websocket_invalid(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_56(%rip), %r12
    leaq 4576(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $31, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_9:
.L12_10:
    leaq 4544(%rbp), %rax
    movq %rax, 120(%rbp)
    leaq 4512(%rbp), %r13
    movq %r13, %r11
    pxor %xmm8, %xmm8
    movups %xmm8, 0(%r11)
    movups %xmm8, 16(%r11)
    movq %r13, %r10
    movb %bl, (%r10)
    leaq 4648(%rbp), %rax
    movq %rax, 112(%rbp)
    movq 112(%rbp), %r10
    movzbl (%r10), %eax
    movl %eax, 104(%rbp)
    movq $1, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movl 104(%rbp), %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movq %r15, (%r10)
    movq %r13, %r10
    movq 120(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    movq 48(%rbp), %rax
    movq $16, %rcx
    movq %rax, %r12
    addq %rcx, %r12
    subq $64, %rsp
    movq %r12, %rax
    movq %rax, 8(%rsp)
    movq 120(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 16(%rsp)
    leaq 4480(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_net_WebSocketSequence_begin
    addq $64, %rsp
    leaq 4480(%rbp), %r14
    movq $24, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movzbl (%r10), %r13d
    testl %r13d, %r13d
    jne .L12_17
    jmp .L12_16
.L12_17:
    leaq 4720(%rbp), %rbx
    movq %r14, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_16:
    subq $96, %rsp
    movq %r12, %rax
    movq %rax, 8(%rsp)
    movq 120(%rbp), %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movups 16(%r10), %xmm8
    movups %xmm8, 16(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movq 136(%rbp), %r10
    leaq 80(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 80(%rsp), %rax
    movq %rax, 24(%rsp)
    movl $1, %eax
    movq %rax, 32(%rsp)
    leaq 4448(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_net_WebSocketSequence_payload
    addq $96, %rsp
    leaq 4448(%rbp), %r13
    movq $24, %rcx
    movq %r13, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L12_20
    jmp .L12_19
.L12_20:
    leaq 4720(%rbp), %rbx
    movq %r13, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_19:
    leaq 4432(%rbp), %rbx
    movq %rbx, %r11
    movq $0, 0(%r11)
    movl $0, 8(%r11)
    movw $0, 12(%r11)
    movl 104(%rbp), %eax
    testl %eax, %eax
    jne .L12_22
    jmp .L12_23
.L12_22:
    movl $128, %eax
    movl %eax, %r12d
    jmp .L12_24
.L12_23:
    movl $0, %eax
    movl %eax, %r12d
.L12_24:
    movzbl %r12b, %r13d
    movl 144(%rbp), %eax
    movl %r13d, %ecx
    movl %eax, %r13d
    orl %ecx, %r13d
    movq %rbx, %r10
    movb %r13b, (%r10)
    movq 128(%rbp), %r10
    movq (%r10), %r13
    movq $126, %rcx
    cmpq %rcx, %r13
    jae .L12_26
.L12_25:
    movq $1, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movzbl %r13b, %r14d
    movq %r12, %r10
    movb %r14b, (%r10)
    movq $2, %rax
    movq %rax, %r12
    jmp .L12_27
.L12_26:
    movq $65536, %rcx
    cmpq %rcx, %r13
    jae .L12_29
.L12_28:
    movq $1, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $126, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq $2, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
.L12_32:
    movq $8, %rcx
    movq %r13, %r14
    shrq %cl, %r14
    movzbl %r14b, %r14d
    movq %r12, %r10
    movb %r14b, (%r10)
    movq $3, %rcx
    movq %rbx, %r14
    addq %rcx, %r14
    movzbl %r13b, %r15d
    movq %r14, %r10
    movb %r15b, (%r10)
    movq $4, %rax
    movq %rax, 32(%rbp)
    jmp .L12_30
.L12_29:
    movq $1, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $127, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq $0, %rax
    movq %rax, %r12
.L12_33:
    movq $8, %rcx
    cmpq %rcx, %r12
    jge .L12_36
.L12_34:
    movq %r12, %rax
    movq $2, %rcx
    addq %rcx, %rax
    jno 1f
    leaq .Ltext_63(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r13
    movq $14, %rcx
    cmpq %rcx, %r13
    jb 1f
    leaq .Ltext_63(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rbx, %rax
    addq %r13, %rax
    movq %rax, 40(%rbp)
    movq 128(%rbp), %r10
    movq (%r10), %r15
    movq $7, %rax
    movq %r12, %rcx
    subq %rcx, %rax
    jno 1f
    leaq .Ltext_63(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r13
    movq %r13, %rax
    movq $8, %rcx
    imulq %rcx, %rax
    jno 1f
    leaq .Ltext_63(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r14
    movq $64, %rcx
    cmpq %rcx, %r14
    jb .L12_38
.L12_37:
    leaq .Ltext_63(%rip), %rdi
    leaq .Ltext_8(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L12_38:
    movq %r14, %rcx
    movq %r15, %r13
    shrq %cl, %r13
    movzbl %r13b, %r13d
    movq 40(%rbp), %r10
    movb %r13b, (%r10)
.L12_35:
    movq $1, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movq %r13, %r12
    jmp .L12_33
.L12_36:
    movq $10, %rax
    movq %rax, 32(%rbp)
.L12_30:
    movq 32(%rbp), %rax
    movq %rax, %r12
.L12_27:
    leaq 4428(%rbp), %rax
    movq %rax, 96(%rbp)
    movq 96(%rbp), %r11
    movl $0, 0(%r11)
    movq 48(%rbp), %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L12_39
    jmp .L12_40
.L12_39:
    subq $32, %rsp
    movq 96(%rbp), %rax
    movq %rax, 0(%rsp)
    movq $4, %rax
    movq %rax, 8(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_net_17websocket_entropy
    addq $32, %rsp
    movl %eax, %r14d
    movl $0, %ecx
    cmpl %ecx, %r14d
    sete %al
    movzbl %al, %r14d
    movl $0, %ecx
    cmpl %ecx, %r14d
    jne .L12_43
.L12_42:
    leaq 4720(%rbp), %rbx
    leaq lb_net_failed(%rip), %r12
    movq %r12, %r10
    movl (%r10), %r12d
    movq %rbx, %r10
    movl %r12d, (%r10)
    leaq .Ltext_65(%rip), %r12
    leaq 4408(%rbp), %r13
    movq %r13, %r10
    movq %r12, (%r10)
    movq $8, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq $43, %rax
    movq %r12, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movq %r13, %r10
    movq %r12, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_43:
.L12_44:
    movq $1, %rcx
    movq %rbx, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movzbl (%r10), %r15d
    movl $128, %ecx
    orl %ecx, %r15d
    movq %r14, %r10
    movb %r15b, (%r10)
    movq $15, %rcx
    cmpq %rcx, %r12
    jb 1f
    leaq .Ltext_67(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $14, %rcx
    cmpq %rcx, %r12
    jbe .L12_46
.L12_47:
    leaq .Ltext_67(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L12_46:
    movq %rbx, %r14
    addq %r12, %r14
    movq $14, %rax
    movq %rax, %r15
    subq %r12, %r15
    leaq 4392(%rbp), %r13
    movq %r13, %r10
    movq %r14, (%r10)
    movq $8, %rcx
    movq %r13, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movq %r15, (%r10)
    leaq 4376(%rbp), %r14
    movq 96(%rbp), %rax
    movq %r14, %r10
    movq %rax, (%r10)
    movq $8, %rcx
    movq %r14, %r15
    addq %rcx, %r15
    movq $4, %rax
    movq %r15, %r10
    movq %rax, (%r10)
    subq $64, %rsp
    movq %r13, %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 0(%rsp)
    movq %r14, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 8(%rsp)
    movq $4, %rax
    movq %rax, 16(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call lb_memory_copy_0g1_u8
    addq $64, %rsp
    movq $4, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    jmp .L12_41
.L12_40:
    movq %r12, %r13
.L12_41:
    leaq 4696(%rbp), %rax
    movq %rax, 88(%rbp)
    movq $15, %rcx
    cmpq %rcx, %r13
    jb 1f
    leaq .Ltext_69(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $0, %rax
    cmpq %r13, %rax
    jbe .L12_48
.L12_49:
    leaq .Ltext_69(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L12_48:
    leaq 4360(%rbp), %r14
    movq %r14, %r10
    movq %rbx, (%r10)
    movq $8, %rcx
    movq %r14, %r15
    addq %rcx, %r15
    movq %r15, %r10
    movq %r13, (%r10)
    subq $64, %rsp
    movq 88(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    movq %r14, %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movq $0, %rax
    movq %rax, 24(%rsp)
    leaq 4328(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_io_9write_all
    addq $64, %rsp
    leaq 4328(%rbp), %r15
    movq $24, %rcx
    movq %r15, %r14
    addq %rcx, %r14
    movq %r14, %r10
    movzbl (%r10), %r14d
    testl %r14d, %r14d
    jne .L12_51
    jmp .L12_50
.L12_51:
    leaq 4720(%rbp), %rbx
    movq %r15, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_50:
    movq 48(%rbp), %r10
    movzbl (%r10), %ebx
    movl $0, %ecx
    cmpl %ecx, %ebx
    jne .L12_54
.L12_53:
    subq $64, %rsp
    movq 88(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    movq 136(%rbp), %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movq $0, %rax
    movq %rax, 24(%rsp)
    leaq 4296(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_io_9write_all
    addq $64, %rsp
    leaq 4296(%rbp), %r13
    movq $24, %rcx
    movq %r13, %rbx
    addq %rcx, %rbx
    movq %rbx, %r10
    movzbl (%r10), %ebx
    testl %ebx, %ebx
    jne .L12_57
    jmp .L12_56
.L12_57:
    leaq 4720(%rbp), %rbx
    movq %r13, %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_56:
    leaq 4720(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_54:
.L12_55:
    leaq 200(%rbp), %rbx
    movq %rbx, %rdi
    xorl %esi, %esi
    movq $4096, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memset
    addq $32, %rsp
    leaq 184(%rbp), %rax
    movq %rax, 72(%rbp)
    movq 72(%rbp), %rax
    movq $8, %rcx
    addq %rcx, %rax
    movq %rax, 64(%rbp)
    leaq 152(%rbp), %rax
    movq %rax, 8(%rbp)
    movq 8(%rbp), %rax
    movq $24, %rcx
    addq %rcx, %rax
    movq %rax, 56(%rbp)
    movq $0, %rax
    movq %rax, 24(%rbp)
.L12_60:
    movq 128(%rbp), %r10
    movq (%r10), %r12
    movq 24(%rbp), %rax
    cmpq %r12, %rax
    jae .L12_62
.L12_61:
    movq 24(%rbp), %rcx
    movq %r12, %r14
    subq %rcx, %r14
    movq $4096, %rcx
    cmpq %rcx, %r14
    jae .L12_64
.L12_63:
    movq %r14, %rax
    movq %rax, 16(%rbp)
    jmp .L12_65
.L12_64:
    movq $4096, %rax
    movq %rax, 16(%rbp)
.L12_65:
    movq $0, %rax
    movq %rax, %r13
.L12_66:
    movq 16(%rbp), %rcx
    cmpq %rcx, %r13
    jae .L12_69
.L12_67:
    movq $4096, %rcx
    cmpq %rcx, %r13
    jb 1f
    leaq .Ltext_71(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rbx, %rax
    addq %r13, %rax
    movq %rax, 0(%rbp)
    movq 24(%rbp), %rax
    movq %r13, %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_71(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r14
    movq 136(%rbp), %r10
    movq (%r10), %rax
    movq %rax, 80(%rbp)
    movq 128(%rbp), %r10
    movq (%r10), %r12
    cmpq %r12, %r14
    jb 1f
    leaq .Ltext_71(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 80(%rbp), %rcx
    movq %r14, %r12
    addq %rcx, %r12
    movq %r12, %r10
    movzbl (%r10), %r12d
.L12_71:
    movq $4, %rcx
    movq %r14, %rax
    xorl %edx, %edx
    divq %rcx
    movq %rdx, %r15
    movq $4, %rcx
    cmpq %rcx, %r15
    jb 1f
    leaq .Ltext_71(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq 96(%rbp), %rax
    movq %r15, %rcx
    movq %rax, %r15
    addq %rcx, %r15
    movq %r15, %r10
    movzbl (%r10), %r15d
    xorl %r12d, %r15d
    movq 0(%rbp), %r10
    movb %r15b, (%r10)
.L12_68:
    movq $1, %rcx
    movq %r13, %r12
    addq %rcx, %r12
    movq %r12, %r13
    jmp .L12_66
.L12_69:
    movq 16(%rbp), %rax
    movq $4097, %rcx
    cmpq %rcx, %rax
    jb 1f
    leaq .Ltext_72(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq $0, %rax
    movq 16(%rbp), %rcx
    cmpq %rcx, %rax
    jbe .L12_72
.L12_73:
    leaq .Ltext_72(%rip), %rdi
    leaq .Ltext_17(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L12_72:
    movq 72(%rbp), %r10
    movq %rbx, (%r10)
    movq 16(%rbp), %rax
    movq 64(%rbp), %r10
    movq %rax, (%r10)
    subq $64, %rsp
    movq 88(%rbp), %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    movq 72(%rbp), %r10
    leaq 48(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 48(%rsp), %rax
    movq %rax, 16(%rsp)
    movq $0, %rax
    movq %rax, 24(%rsp)
    leaq 152(%rbp), %rax
    movq %rax, (%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    movq 24(%rsp), %r9
    call lb_io_9write_all
    addq $64, %rsp
    movq 56(%rbp), %r10
    movzbl (%r10), %r12d
    testl %r12d, %r12d
    jne .L12_75
    jmp .L12_74
.L12_75:
    leaq 4720(%rbp), %rbx
    movq 8(%rbp), %r10
    movq %rbx, %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 16(%r10), %rax
    movq %rax, 16(%r11)
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $1, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
.L12_74:
    movq 24(%rbp), %rax
    movq 16(%rbp), %rcx
    addq %rcx, %rax
    jnc 1f
    leaq .Ltext_73(%rip), %rdi
    leaq .Ltext_77(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
1:
    movq %rax, %r12
    movq %r12, %rax
    movq %rax, 24(%rbp)
    jmp .L12_60
.L12_62:
    leaq 4720(%rbp), %rbx
    movq $24, %rcx
    movq %rbx, %r12
    addq %rcx, %r12
    movl $0, %eax
    movq %r12, %r10
    movb %al, (%r10)
    movq %rbx, %rsi
    movq 4792(%rbp), %rdi
    movq $32, %rdx
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call memcpy
    addq $32, %rsp
    movq 4792(%rbp), %rax
    movq 4968(%rbp), %rdi
    movq 4960(%rbp), %rsi
    movdqu 4944(%rbp), %xmm6
    movdqu 4928(%rbp), %xmm7
    movdqu 4912(%rbp), %xmm8
    movdqu 4896(%rbp), %xmm9
    movdqu 4880(%rbp), %xmm10
    movdqu 4864(%rbp), %xmm11
    movdqu 4848(%rbp), %xmm12
    movdqu 4832(%rbp), %xmm13
    movdqu 4816(%rbp), %xmm14
    movdqu 4800(%rbp), %xmm15
    movq 4784(%rbp), %rbx
    movq 4776(%rbp), %r12
    movq 4768(%rbp), %r13
    movq 4760(%rbp), %r14
    movq 4752(%rbp), %r15
    leaq 4976(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .section .text$lb_memory_copy_0g1_u8,"xr"
    .p2align 4
    .globl lb_memory_copy_0g1_u8
lb_memory_copy_0g1_u8:
    .seh_proc lb_memory_copy_0g1_u8
    pushq %rbp
    .seh_pushreg %rbp
    subq $272, %rsp
    .seh_stackalloc 272
    movq %rsp, %rbp
    .seh_setframe %rbp, 0
    movq %rdi, 264(%rbp)
    .seh_savereg %rdi, 264
    movq %rsi, 256(%rbp)
    .seh_savereg %rsi, 256
    movdqu %xmm6, 240(%rbp)
    .seh_savexmm %xmm6, 240
    movdqu %xmm7, 224(%rbp)
    .seh_savexmm %xmm7, 224
    movdqu %xmm8, 208(%rbp)
    .seh_savexmm %xmm8, 208
    movdqu %xmm9, 192(%rbp)
    .seh_savexmm %xmm9, 192
    movdqu %xmm10, 176(%rbp)
    .seh_savexmm %xmm10, 176
    movdqu %xmm11, 160(%rbp)
    .seh_savexmm %xmm11, 160
    movdqu %xmm12, 144(%rbp)
    .seh_savexmm %xmm12, 144
    movdqu %xmm13, 128(%rbp)
    .seh_savexmm %xmm13, 128
    movdqu %xmm14, 112(%rbp)
    .seh_savexmm %xmm14, 112
    movdqu %xmm15, 96(%rbp)
    .seh_savexmm %xmm15, 96
    movq %rbx, 88(%rbp)
    .seh_savereg %rbx, 88
    movq %r12, 80(%rbp)
    .seh_savereg %r12, 80
    movq %r13, 72(%rbp)
    .seh_savereg %r13, 72
    movq %r14, 64(%rbp)
    .seh_savereg %r14, 64
    .seh_endprologue
    movq %rcx, 288(%rbp)
    movq %rdx, 296(%rbp)
    movq %r8, 304(%rbp)
    movq %r9, 312(%rbp)
    movq 288(%rbp), %r10
    leaq 48(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 296(%rbp), %r10
    leaq 32(%rbp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    movq 304(%rbp), %rax
    movq %rax, 16(%rbp)
    leaq 16(%rbp), %rbx
    movq %rbx, %r10
    movq (%r10), %rbx
    leaq 48(%rbp), %r12
    movq $8, %rcx
    movq %r12, %r13
    addq %rcx, %r13
    movq %r13, %r10
    movq (%r10), %r13
    cmpq %r13, %rbx
    seta %al
    movzbl %al, %r13d
    testl %r13d, %r13d
    jne .L13_11
    jmp .L13_4
.L13_11:
    movl %r13d, %r14d
    jmp .L13_5
.L13_4:
    leaq 32(%rbp), %r13
    movq $8, %rcx
    addq %rcx, %r13
    movq %r13, %r10
    movq (%r10), %r13
    cmpq %r13, %rbx
    seta %al
    movzbl %al, %r14d
.L13_5:
    movzbl %r14b, %r13d
    testl %r13d, %r13d
    jne .L13_1
    jmp .L13_2
.L13_1:
    leaq .Ltext_17(%rip), %r13
    leaq 0(%rbp), %r14
    movq %r14, %r10
    movq %r13, (%r10)
    movq $8, %rcx
    movq %r14, %r13
    addq %rcx, %r13
    movq $19, %rax
    movq %r13, %r10
    movq %rax, (%r10)
    leaq .Ltext_74(%rip), %r13
    subq $48, %rsp
    movq %r13, %rax
    movq %rax, 0(%rsp)
    movq %r14, %r10
    leaq 32(%rsp), %r11
    movups 0(%r10), %xmm8
    movups %xmm8, 0(%r11)
    leaq 32(%rsp), %rax
    movq %rax, 8(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    call lb_core_12trap_text_at
    addq $48, %rsp
    jmp .L13_3
.L13_2:
.L13_3:
    movq $0, %rcx
    cmpq %rcx, %rbx
    jbe .L13_7
.L13_6:
    movq %r12, %r10
    movq (%r10), %r13
    leaq 32(%rbp), %r14
    movq %r14, %r10
    movq (%r10), %r14
    subq $32, %rsp
    movq %r13, %rax
    movq %rax, 0(%rsp)
    movq %r14, %rax
    movq %rax, 8(%rsp)
    movq %rbx, %rax
    movq %rax, 16(%rsp)
    movq 0(%rsp), %rcx
    movq 8(%rsp), %rdx
    movq 16(%rsp), %r8
    call memcpy
    addq $32, %rsp
    movq %rax, %r13
    testq %r13, %r13
    jne .L13_9
    jmp .L13_10
.L13_10:
    leaq .Ltext_75(%rip), %rdi
    leaq .Ltext_76(%rip), %rsi
    movq %rdx, %r8
    movq %rsi, %rdx
    movq %rdi, %rcx
    subq $32, %rsp
    call lb_core_7trap_at
    addq $32, %rsp
.L13_9:
    jmp .L13_8
.L13_7:
.L13_8:
    movq 264(%rbp), %rdi
    movq 256(%rbp), %rsi
    movdqu 240(%rbp), %xmm6
    movdqu 224(%rbp), %xmm7
    movdqu 208(%rbp), %xmm8
    movdqu 192(%rbp), %xmm9
    movdqu 176(%rbp), %xmm10
    movdqu 160(%rbp), %xmm11
    movdqu 144(%rbp), %xmm12
    movdqu 128(%rbp), %xmm13
    movdqu 112(%rbp), %xmm14
    movdqu 96(%rbp), %xmm15
    movq 88(%rbp), %rbx
    movq 80(%rbp), %r12
    movq 72(%rbp), %r13
    movq 64(%rbp), %r14
    leaq 272(%rbp), %rsp
    popq %rbp
    ret
    .seh_endproc

    .text

    .section .CRT$XCU,"dr"
    .p2align 3
    .quad lb_net_15websocket_frame_0init
    .text

    .section .rdata,"dr"
    .p2align 4
.Lvsign32:
    .long 2147483648, 2147483648, 2147483648, 2147483648
.Lvsign64:
    .quad -9223372036854775808, -9223372036854775808
.Ltext_0:
    .asciz "src/std/net/websocket/frame.lucb:24:5"
.Ltext_1:
    .asciz "WebSocket reserved bits require an extension"
.Ltext_2:
    .asciz "src/std/net/websocket/frame.lucb:26:5"
.Ltext_3:
    .asciz "src/std/net/websocket/frame.lucb:27:5"
.Ltext_4:
    .asciz "src/std/net/websocket/frame.lucb:28:5"
.Ltext_5:
    .asciz "WebSocket masking is wrong for this direction"
.Ltext_6:
    .asciz "src/std/net/websocket/frame.lucb:31:5"
.Ltext_7:
    .asciz "src/std/net/websocket/frame.lucb:37:9"
.Ltext_8:
    .asciz "shift count out of range"
.Ltext_9:
    .asciz "nonminimal WebSocket payload length"
.Ltext_10:
    .asciz "src/std/net/websocket/frame.lucb:44:9"
.Ltext_11:
    .asciz "WebSocket payload length has its sign bit set"
.Ltext_12:
    .asciz "src/std/net/websocket/frame.lucb:48:13"
.Ltext_13:
    .asciz "WebSocket frame exceeds its byte limit"
.Ltext_14:
    .asciz "WebSocket control frame must be final and at most 125 bytes"
.Ltext_15:
    .asciz "src/std/net/websocket/frame.lucb:58:9"
.Ltext_16:
    .asciz "src/std/net/websocket/frame.lucb:60:9"
.Ltext_17:
    .asciz "index out of bounds"
.Ltext_18:
    .asciz "src/std/net/websocket/frame.lucb:61:9"
.Ltext_19:
    .asciz "unreachable"
.Ltext_20:
    .asciz "src/std/net/websocket/frame.lucb:62:5"
.Ltext_21:
    .asciz "reserved WebSocket opcode"
.Ltext_22:
    .asciz "src/std/net/websocket/frame.lucb:72:9"
.Ltext_23:
    .asciz "invalid WebSocket close payload length"
.Ltext_24:
    .asciz "src/std/net/websocket/frame.lucb:81:5"
.Ltext_25:
    .asciz "reserved WebSocket close status"
.Ltext_26:
    .asciz "src/std/net/websocket/frame.lucb:85:5"
.Ltext_27:
    .asciz "WebSocket data follows a close frame"
.Ltext_28:
    .asciz "WebSocket continuation has no initial frame"
.Ltext_29:
    .asciz "new WebSocket message interrupts fragmentation"
.Ltext_30:
    .asciz "src/std/net/websocket/frame.lucb:115:9"
.Ltext_31:
    .asciz "WebSocket message exceeds its byte limit"
.Ltext_32:
    .asciz "src/std/net/websocket/frame.lucb:117:9"
.Ltext_33:
    .asciz "src/std/net/websocket/frame.lucb:123:17"
.Ltext_34:
    .asciz "src/std/net/websocket/frame.lucb:124:17"
.Ltext_35:
    .asciz "src/std/net/websocket/frame.lucb:126:21"
.Ltext_36:
    .asciz "src/std/net/websocket/frame.lucb:162:9"
.Ltext_37:
    .asciz "WebSocket decoder has failed"
.Ltext_38:
    .asciz "WebSocket output buffer must be nonempty"
.Ltext_39:
    .asciz "src/std/net/websocket/frame.lucb:172:13"
.Ltext_40:
    .asciz "WebSocket ended without a complete close frame"
.Ltext_41:
    .asciz "src/std/net/websocket/frame.lucb:188:13"
.Ltext_42:
    .asciz "src/std/net/websocket/frame.lucb:189:13"
.Ltext_43:
    .asciz "src/std/net/websocket/frame.lucb:190:13"
.Ltext_44:
    .asciz "src/std/net/websocket/frame.lucb:191:13"
.Ltext_45:
    .asciz "src/std/net/websocket/frame.lucb:200:9"
.Ltext_46:
    .asciz "src/std/net/websocket/frame.lucb:203:9"
.Ltext_47:
    .asciz "src/std/net/websocket/frame.lucb:204:13"
.Ltext_48:
    .asciz "src/std/net/websocket/frame.lucb:206:13"
.Ltext_49:
    .asciz "src/std/net/websocket/frame.lucb:207:13"
.Ltext_50:
    .asciz "division by zero"
.Ltext_51:
    .asciz "src/std/net/websocket/frame.lucb:208:9"
.Ltext_52:
    .asciz "src/std/net/websocket/frame.lucb:210:9"
.Ltext_53:
    .asciz "src/std/net/websocket/frame.lucb:213:9"
.Ltext_54:
    .asciz "src/std/net/websocket/frame.lucb:226:9"
.Ltext_55:
    .asciz "WebSocket encoder has failed"
.Ltext_56:
    .asciz "invalid WebSocket control frame"
.Ltext_57:
    .asciz "src/std/net/websocket/frame.lucb:246:9"
.Ltext_58:
    .asciz "src/std/net/websocket/frame.lucb:249:13"
.Ltext_59:
    .asciz "src/std/net/websocket/frame.lucb:251:13"
.Ltext_60:
    .asciz "src/std/net/websocket/frame.lucb:252:13"
.Ltext_61:
    .asciz "src/std/net/websocket/frame.lucb:253:13"
.Ltext_62:
    .asciz "src/std/net/websocket/frame.lucb:256:13"
.Ltext_63:
    .asciz "src/std/net/websocket/frame.lucb:258:17"
.Ltext_64:
    .asciz "src/std/net/websocket/frame.lucb:262:13"
.Ltext_65:
    .asciz "WebSocket masking randomness is unavailable"
.Ltext_66:
    .asciz "src/std/net/websocket/frame.lucb:264:13"
.Ltext_67:
    .asciz "src/std/net/websocket/frame.lucb:265:13"
.Ltext_68:
    .asciz "src/std/net/websocket/frame.lucb:266:13"
.Ltext_69:
    .asciz "src/std/net/websocket/frame.lucb:267:9"
.Ltext_70:
    .asciz "src/std/net/websocket/frame.lucb:274:13"
.Ltext_71:
    .asciz "src/std/net/websocket/frame.lucb:276:17"
.Ltext_72:
    .asciz "src/std/net/websocket/frame.lucb:277:13"
.Ltext_73:
    .asciz "src/std/net/websocket/frame.lucb:278:13"
.Ltext_74:
    .asciz "src/std/memory.lucb:328:9"
.Ltext_75:
    .asciz "src/std/memory.lucb:330:9"
.Ltext_76:
    .asciz "null_foreign"
.Ltext_77:
    .asciz "integer overflow"

    .section .rdata,"dr"
    .p2align 3
