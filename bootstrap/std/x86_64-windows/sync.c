...
void lb_sync_8sleep_on(void* lb_address, uint32_t lb_value) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    {
        lb_pos = "src/std/sync.lucb:22:9";
        uint32_t lb_expected __attribute__((unused)) = lb_value;
        lb_pos = "src/std/sync.lucb:23:9";
        (void)(((void)(lb_x_sync_WaitOnAddress(lb_address, &(lb_expected), 4ULL, 4294967295ULL))));
    }
}
void lb_sync_8wake_one(void* lb_address) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    {
        lb_pos = "src/std/sync.lucb:32:9";
        (void)(lb_x_sync_WakeByAddressSingle(lb_address));
    }
}
void lb_sync_10wake_every(void* lb_address) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    {
        lb_pos = "src/std/sync.lucb:40:9";
        (void)(lb_x_sync_WakeByAddressAll(lb_address));
    }
}
