...
lb_r_gpu_Backend lb_gpu_14select_backend(uint8_t lb_requested) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    {
        lb_pos = "src/std/gpu/backend.lucb:10:9";
        if (!!(((lb_requested == ((uint8_t)0u)) || (lb_requested == ((uint8_t)2u))))) 
        {
            lb_pos = "src/std/gpu/backend.lucb:11:13";
            uint8_t _lb_ret7 = ((uint8_t)2u);
...
lb_r_void_0p lb_gpu_19backend_device_open(uint8_t lb_backend) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    lb_pos = "src/std/gpu/backend.lucb:18:5";
    if (!!((lb_platform_windows && (lb_backend == ((uint8_t)2u))))) 
    {
        lb_pos = "src/std/gpu/backend.lucb:19:9";
        void* _lb_ret9 = ((void*)((({ lb_r_gpu_VulkanDevice_0p _lb_r10 = lb_gpu_18vulkan_device_open(); if (_lb_r10.failed) {
...
void lb_gpu_22backend_device_destroy(uint8_t lb_backend, void* lb_native) {
    const char* lb_saved_pos __attribute__((cleanup(lb_restore_pos), unused)) = lb_pos;
    lb_pos = "src/std/gpu/backend.lucb:26:5";
    if (!!((lb_platform_windows && (lb_backend == ((uint8_t)2u))))) 
    {
        lb_pos = "src/std/gpu/backend.lucb:27:9";
        (void)(lb_gpu_21vulkan_device_destroy(((struct lb_gpu_VulkanDevice*)(lb_native))));
        lb_pos = "src/std/gpu/backend.lucb:28:9";
        return;
    }
    {
    }
