...
# Usage: programs/editor/check.sh [luce-base binary]
set -eu
cd "$(dirname "$0")/../.."
LB=${1:-./build/luce-base}
"$LB" build programs/editor/main.lucb --native -o build/editor-check
printf 'line one\nline two\nline three\n' > build/editor-check.txt
./build/editor-check build/editor-check.txt --replay programs/editor/check.keys > build/editor-check.screen
if [ "$(cat build/editor-check.txt)" != "$(printf 'okfour\nline two\nline three')" ]; then
    echo "FAIL: the saved text"; cat build/editor-check.txt; exit 1
fi
if ! cmp -s build/editor-check.screen programs/editor/check.screen; then
