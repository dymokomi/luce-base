...
# Usage: programs/http/check.sh [luce-base binary]
set -eu
cd "$(dirname "$0")/../.."
LB=${1:-./build/luce-base}
PORT=18090
"$LB" build programs/http/main.lucb --native -o build/http-check
./build/http-check --port $PORT --root programs/http/www --threads 4 > build/http-check.log 2>&1 &
SERVER=$!
sleep 1
