...
if [ "$(uname -s)" = Darwin ]; then
    ...
    sudo apt-get update
    sudo apt-get install -y clang gcc mingw-w64 cmake ninja-build pkg-config curl gdb binutils llvm libx11-dev libxext-dev libxrandr-dev libxcursor-dev libxi-dev libxfixes-dev libxss-dev
