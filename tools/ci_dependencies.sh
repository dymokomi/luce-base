...
if [ "$(uname -s)" = Darwin ]; then
    ...
    sudo apt-get update
    sudo apt-get install -y clang cmake ninja-build pkg-config curl
    git clone --depth 1 --branch release-3.2.16 https://github.com/libsdl-org/SDL.git ../sdl3
    cmake -S ../sdl3 -B ../sdl3/build -G Ninja -DCMAKE_BUILD_TYPE=Release -DSDL_SHARED=ON -DSDL_STATIC=OFF -DSDL_TESTS=OFF -DSDL_X11=OFF -DSDL_WAYLAND=OFF
