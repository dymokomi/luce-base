...
#   lib/luce-base/HOST/libstd.a       the standard library, native backend
#   lib/luce-base/HOST/libstd-c.a     the standard library, C backend
...
# runs from wherever it is unpacked; a program it builds links the library statically and
...
library="build/lib/luce-base/$host"
for f in "$exe" "$library/libstd.a" "$library/libstd-c.a"; do
...
work="$out/tree"
rm -rf "$work"
mkdir -p "$work/$tree/bin" "$work/$tree/lib/luce-base/$host" "$work/$tree/share/luce-base/docs/language"
