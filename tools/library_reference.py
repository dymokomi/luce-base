...
doc = ["# The Luce Base standard library\n",
       ...
       "Everything here is Base over `extern` declarations of the C library; where a constant or a layout differs by target it is a branch on the `platform` module (§19.5), proved on each host by `tests/platform`.\n"]
for n in names:
    if n in skip or n == "c":
        continue
    doc.append(render(n))
